...
## Corrección: menú de "•••" (Cancelar pedido) en /app/pedidos/ no se mostraba (2026-09-23)

- El desarrollador (como Administrador) reportó que el botón de 3 puntos en la lista de Pedidos no hacía nada al hacer clic. Se descartó primero el permiso (`order.can_cancel` exige rol Administrador y que el pedido no esté Cancelado/Recogido/Entregado — el desarrollador confirmó ser Admin) y el propio desarrollador identificó que era CSS, no permisos ni estado.
- Diagnóstico con Playwright: el `<details class="order-more-actions">` sí se abre (`el.open === true`) y su menú (`.order-more-actions-menu`) tiene `display:block; opacity:1` con una caja real — pero `document.elementFromPoint()` en esa posición devolvía OTRO elemento (una tarjeta no relacionada), confirmando que el menú estaba pintado pero recortado/invisible en la práctica. Se recorrieron los ancestros comprobando `overflow` de cada uno y se encontró `section.order-row-actions` con `overflow: hidden`.
- Causa raíz, confirmada iterando `document.styleSheets` y `element.matches(selector)` (técnica ya documentada esta semana para este archivo): la regla `.order-board-primary > section, .order-board-secondary > section { overflow: hidden; ... }` (pensada para que las tarjetitas de Folio/Total/Modalidad/Forma de pago recorten texto largo con esquinas redondeadas) también matcheaba `.order-row-actions` — que es OTRO `<section>` hijo directo ahí, pero con un trabajo distinto (contener el menú desplegable "•••", que necesita salirse de su caja para mostrarse encima del contenido).
- Corrección: se agregó `.order-board-primary > .order-row-actions, .order-board-secondary > .order-row-actions { overflow: visible; }` justo después de la definición base de `.order-row-actions` (línea ~1147). Con 2 clases en la cadena del selector le gana en especificidad a la regla original (1 clase + 1 elemento) sin necesitar `!important`. No se tocó la regla original (las tarjetitas de info siguen recortando texto normalmente).
- Verificado con Playwright: `overflow` computado de `.order-row-actions` pasó de `hidden` a `visible`; `elementFromPoint()` en la posición del menú ahora devuelve el botón real "Cancelar pedido" en vez de una tarjeta ajena. Confirmado también con captura de pantalla. Se subió `app.css?v=240` a `?v=241`. Se ejecutó la suite completa (78 pruebas) sin fallos; no se requirió migración.
- Nota para el futuro: este es otro caso del patrón "una regla genérica con selector compuesto por hermanos (`A > section, B > section`) atrapa sin querer a un elemento que técnicamente coincide con el selector pero tiene un propósito distinto" — al depurar un elemento que "no aparece" pese a `display`/`opacity` correctos, revisar `overflow` en TODA la cadena de ancestros (no sólo el padre inmediato), y usar `document.elementFromPoint()` para confirmar qué se está pintando realmente ahí, no sólo confiar en `getComputedStyle` del propio elemento.

## Nueva regla: el Repartidor puede cerrar su propia entrega (solo Terminal/Transferencia) y registrar propina de Terminal aunque ya esté Entregado (2026-09-23)

- Pedido del desarrollador, tras varias rondas de preguntas sobre cómo funcionaba el flujo de status/Caja/propinas (documentadas en las conversaciones del día): cambiar el flujo para que el Repartidor pueda marcar su propio pedido como "Entregado" — algo que antes estaba bloqueado explícitamente en el código pese a que la capa de servicio (`transition_order`) ya lo permitía por debajo.
- **Decisiones confirmadas con el desarrollador antes de tocar código** (importante para no revertir esto sin querer):
  - **Efectivo queda excluido a propósito**: el flujo de efectivo sigue exactamente igual — solo Caja, con "Cambio devuelto" (`confirm_cash_settlement`), puede llevar una entrega en efectivo hasta "Entregado". El Repartidor NO puede auto-completar pedidos en efectivo.
  - **"Liberar" no cambia**: sigue llevando el pedido a "En reparto" con un clic, igual que hoy, para cualquier método de pago.
  - **Propina de Terminal**: el Repartidor gana una ventana más amplia — puede registrarla mientras el pedido está "En reparto" **o** ya "Entregado" (antes solo "En reparto"), pero sigue siendo una sola vez por pedido, y sigue exclusiva de Efectivo/Terminal (Transferencia la sigue registrando Telefonista/Admin aparte, sin cambios).
  - **`restart_cycle` NO se toca a propósito**: no limpia `delivery_tip_updated_at` al reiniciar un ciclo (confirmado que esto ya era así). El desarrollador decidió dejarlo así deliberadamente: si se reinicia el ciclo, el Repartidor pierde su oportunidad de propina (porque el campo sigue con la fecha del ciclo anterior) y sólo Admin puede corregirla después. No es un bug, es la regla que quiere.
- **Cambios de código** (`orders/views.py`):
  - `delivery_complete`: el bloqueo total para el perfil Repartidor se reemplazó por una condición — se le permite completar (`complete_delivery`, nunca `dispatch_delivery`, nunca `restart_cycle`) sólo si el pedido es el suyo (`delivery_person_id == request.user.pk`) y el método de pago es Terminal o Transferencia. Efectivo sigue devolviendo 403 igual que antes.
  - `_can_edit_delivery_tip_on_board`: para el perfil Repartidor, ahora la ventana de estado depende del método de pago — Efectivo exige `OUT_FOR_DELIVERY` (sin cambios); Terminal permite `OUT_FOR_DELIVERY` **o** `DELIVERED`. Se mantiene sin cambios: debe ser su pedido asignado y `delivery_tip_updated_at is None` (una sola vez).
  - `delivery_tip_update` (vista): se quitó un bloqueo redundante que rechazaba cualquier propina con el pedido en Entregado/Recogido sin importar el método de pago — ya quedaba mal para Terminal con la nueva regla; `_can_edit_delivery_tip_on_board` (arriba en la misma función) ya decide correctamente, así que sólo hacía falta borrar el bloqueo duplicado y desactualizado.
  - `delivery_board` (vista, listado que arma cada tarjeta del tablero de Repartos): `order.can_update_delivery_status` (controla si se dibuja el botón "Marcar como entregado" en la plantilla) tenía la misma regla hardcodeada a `False` para Repartidor; se corrigió para permitirlo sólo si el pedido sigue "En reparto" y el método de pago es Terminal o Transferencia. `order.can_edit_delivery_tip` ya llamaba a `_can_edit_delivery_tip_on_board`, así que se corrigió solo al arreglar esa función.
  - `update_delivery_tip` (servicio, `orders/services.py`) **no necesitó ningún cambio**: nunca tuvo una restricción de status para empezar (sólo revisa que sea el pedido asignado, que no se haya registrado antes, y Efectivo/Terminal) — el bloqueo por status solo existía en la capa de vista, ya corregida arriba.
- **Pruebas nuevas** (`orders/tests.py`, clase `DeliveryProfileRestrictionTests`, 6 pruebas agregadas): repartidor completa su propia entrega Terminal/Transferencia (200, status pasa a Entregado); repartidor sigue sin poder completar su propia entrega en Efectivo (403, status no cambia); repartidor no puede completar el pedido de otro repartidor (403); repartidor registra propina de Terminal con el pedido ya Entregado (200, se guarda); repartidor sigue sin poder registrar propina de Efectivo una vez Entregado (403). Se ejecutó la suite completa sin fallos (83 pruebas en total, las 77 anteriores + las 6 nuevas).

## Actualización: agente de impresión de la Dell reutiliza un solo navegador Edge en vez de abrir uno por ticket (2026-09-23)

- Causa raíz del fallo real en producción (ticket llegaba a "enviado" en verde pero terminaba en rojo/fallido, visible en el campo "Error" del `PrintJob` en el admin): `agent.py::render_ticket()` abría y cerraba Edge por completo (`sync_playwright().start()` + `chromium.launch()`) en cada ticket impreso. Bajo uso normal, decenas de veces al día, Edge a veces tardaba en arrancar o Windows lo cerraba a medio lanzar; Playwright reportaba `BrowserType.launch: Target page, context or browser has been closed`.
- Arreglo: `agent.py::run()` ahora abre Playwright y lanza Edge **una sola vez** al iniciar (`playwright = sync_playwright().start()`, `browser = playwright.chromium.launch(channel="msedge", headless=True)`) y lo reutiliza durante todo el turno; `render_ticket(browser, html)` cambió de firma para recibir ese navegador ya abierto y sólo abrir/cerrar una `Page` ligera por ticket, en vez de un navegador completo. Se agregó `browser_alive()` (chequea `browser.is_connected()`) que se llama antes y después de cada trabajo — si Edge se cerró solo por cualquier motivo, se vuelve a abrir automáticamente antes del siguiente ticket, en vez de que el agente se quede fallando todos los trabajos hasta que alguien lo reinicie a mano. El `run()` cierra el navegador y detiene Playwright en un bloque `finally` al salir.
- Validado sólo con `py_compile` en este entorno (sin acceso directo a la Dell ni a la impresora); el desarrollador aplicó el cambio manualmente en la Dell (edición directa del archivo con respaldo previo) y quedó pendiente de confirmar el comportamiento bajo uso real. **Verificar con el desarrollador si esto resolvió los fallos "verde luego rojo" antes de dar este tema por cerrado.**

## Pendiente de diseño: reglas de vinculación de pedidos a cortes de terminal/transferencia + advertencia de datos no coincidentes (anotado 2026-09-23, sin implementar)

Reglas de negocio confirmadas por el desarrollador para una futura vinculación de pedidos con registros de Clover/Mercado Pago/Terminal/Transferencia en Caja → Propinas. **Todavía no se ha tocado código para esto** — queda documentado aquí como referencia antes de implementar.

- **Restricción general**: un pedido en status **Cancelado nunca debe poder vincularse** a ningún tipo de corte (Clover, Mercado Pago, Terminal genérico, Transferencia), sin excepción.
- **Reglas por tipo de corte** (qué pedidos son vinculables, basta con que cumplan la condición de su propio tipo):
  - **Clover**: sólo pedidos de **Mesa** (`TableAccount`) con la cuenta ya **cerrada**.
  - **Mercado Pago**: sólo pedidos de **Entrega a domicilio** (`order_type=DELIVERY`) **y** status **Entregado** (`DELIVERED`) — deben cumplirse ambas condiciones a la vez.
  - **Terminal** (genérico): la unión de los tres casos — Mesa con cuenta cerrada, **Recoger** con status **Recogido** (`PICKED_UP`), y **Entrega a domicilio** con status **Entregado** (`DELIVERED`).
- **Nueva funcionalidad pendiente de diseñar y confirmar antes de programar**: cuando se registra un cobro (Clover, Mercado Pago o Transferencia) con total cobrado, propina y "propina para" (nombre), y se vincula a un pedido cuyos valores ya guardados no coinciden con lo recién capturado, se debe mostrar una ventana de advertencia que señale exactamente qué campos difieren (valor anterior vs. nuevo) y ofrezca tres opciones: sobrescribir el registro existente con los nuevos valores, guardar como un registro nuevo separado, o cancelar la vinculación. Pendiente: investigar el modelo `TerminalMovement` (`orders/models.py`) y las vistas `cashier_terminal_board`/`cashier_terminal_movement_save` (`orders/views.py`) antes de proponer el diseño concreto — por instrucción explícita del desarrollador para este tema, el diseño se debe confirmar con él en palabras antes de escribir cualquier código.

## Implementado: reglas de vinculación de pedidos/mesas por tipo de corte de terminal (2026-09-23)

Al investigar el código para diseñar la funcionalidad anotada arriba, se encontró que **no existe un cuarto proveedor "Terminal"** en `TerminalCut.Provider` — sólo `clover`, `mercado_pago` y `transfer`. El desarrollador aclaró que su "Terminal" del punto 1 se refería a Clover + Mercado Pago juntos (ya existente, sólo para el cálculo de "Aplicación" esperado vs. cobrado), y corrigió el punto 2 con las reglas reales por proveedor, que ya quedaron implementadas:

- **Clover**: sólo se puede vincular una **Mesa** (`TableAccount`) con `status=Cerrada` y `payment_method=Terminal`. Ya no ofrece pedidos de Entrega (antes sí los ofrecía, junto con Mercado Pago, sin distinción — eso era el comportamiento viejo).
- **Mercado Pago**: sólo se puede vincular un **pedido de Entrega a domicilio** con `payment_method=Terminal` y `status=Entregado`. Ya no ofrece Mesas (antes sí las ofrecía).
- **Transferencias**: se puede vincular tanto **Mesas** (`status=Cerrada`, `payment_method=Transferencia`, antes no ofrecía mesas en absoluto) como **pedidos de Entrega a domicilio** con `payment_method=Transferencia` y `status` en {En reparto, Entregado} (antes sólo aceptaba Entregado).
- Ningún proveedor ofrece pedidos de **Recoger** para vincular — no se pidió para ninguno de los tres.
- La restricción "un pedido Cancelado nunca se puede vincular" no necesitó código aparte: como cada filtro exige un status específico (Cerrada/Entregado/En reparto), un registro Cancelado nunca cumple ninguna de esas condiciones.
- Cambios: `orders/views.py` (`cashier_terminal_board` arma `linked_orders`/`linked_tables` según el proveedor; `cashier_terminal_movement_save` valida lo mismo del lado del servidor, con mensajes de error específicos si se intenta vincular algo no permitido para ese proveedor) y `orders/templates/orders/cashier_terminal_board.html` (se quitó el `{% if not is_transfer_provider %}` que ocultaba siempre el optgroup de Mesas en Transferencias; ahora el optgroup se llena o queda vacío según lo que devuelva la vista, y su etiqueta usa `{{ expected_payment_label }}` en vez de un texto fijo "Terminal").
- **Pruebas nuevas** (`orders/tests.py`, clase `TerminalMovementLinkingRulesTests`, 7 pruebas): Clover vincula mesa cerrada con Terminal pero rechaza un pedido de Entrega; Mercado Pago vincula pedido Entregado con Terminal pero rechaza una mesa; Mercado Pago rechaza un pedido todavía en reparto; Transferencias vincula pedidos tanto en Entregado como En reparto (con Transferencia); Transferencias vincula una mesa cerrada con Transferencia; Transferencias rechaza una mesa pagada con Terminal; un pedido Cancelado nunca aparece en la lista de vinculables. Suite completa verificada sin fallos (90 pruebas en total, las 83 anteriores + las 7 nuevas).
- **Sigue pendiente**: la ventana de advertencia de "datos no coincidentes" (sobrescribir/nuevo registro/cancelar) descrita en la sección anterior — eso no se tocó todavía, sólo las reglas de qué se puede vincular.

## Implementado: diálogo de "Vincular" con detección de datos no coincidentes (sobrescribir/nuevo/cancelar) (2026-09-24)

- Se reemplazó el `<select name="linked_record">` de cada renglón en Caja → Terminales por un botón que abre un `<dialog>` nativo compartido (deliberadamente aislado con su propio CSS chico, no el cascade gigante de `app.css`, para evitar los problemas de especificidad que ya nos habían mordido antes en este archivo).
- Al abrir el diálogo se listan los pedidos/mesas vinculables (ya filtrados por las reglas de Clover/Mercado Pago/Transferencias) marcando ✓/⚠ según si Total, Propina y Propina-para coinciden con lo que ya está escrito en esa fila. Click en un candidato: si coincide todo (o la fila está vacía) vincula de inmediato; si no coincide, detiene la acción y muestra una tabla de diferencias con 3 botones: **Sobrescribir** (aplica los datos del pedido/mesa a la fila actual), **Guardar como nuevo** (la fila actual no se toca; se crea una fila/movimiento nuevo aparte con los datos del pedido/mesa), **Cancelar** (no cambia nada).
- `orders/views.py: cashier_terminal_board` ya no arma listas "disponibles" por renglón en el servidor (`movement.available_link_orders`/`available_link_tables` se eliminaron); en su lugar manda una sola lista completa `link_candidates` (con su `movement_id` si ya está tomado) vía `json_script`, y el JS filtra al vuelo comparando contra el vínculo actual de cada fila en pantalla — así un vínculo recién usado desaparece de las demás filas sin recargar la página.
- `static/js/cashier-terminal-board.js` (`?v=8`) quedó bastante simplificado: se eliminó `refreshLinkedOptions()` y el manejo especial de `.app-select` en `appendBlankFrom` (ya no hay ningún `<select>` dentro de cada fila).
- **Bug encontrado y corregido en vivo por el desarrollador probando en local**: varios botones nuevos del diálogo (la opción "Sin vincular", el nombre del candidato, el botón "×" de cerrar, y "Guardar como nuevo") se veían como una barra vacía — texto blanco sobre fondo blanco. Causa: el `app.css` de este proyecto tiene una regla global `button { color: white; background: #285943; ... }` (line ~732) que aplica a TODO `<button>` salvo que se sobreescriba; las clases nuevas (`.terminal-link-candidate`, `.terminal-link-dialog-close`, `[data-link-conflict-new]`) sólo sobreescribían `background`, no `color`, así que el texto blanco seguía ganando contra el nuevo fondo blanco. Se corrigió agregando `color` explícito a las tres. **Lección para futuros botones nuevos en este proyecto: siempre revisar/sobreescribir `color` explícitamente, nunca asumir que un fondo claro nuevo implica texto oscuro por defecto**, por esa regla global de `button`.
- `app.css` quedó en `?v=243` en `templates/base_internal.html` tras los ajustes de color.

## Nueva regla: quién se queda como responsable de la mesa (y su propina) se confirma explícitamente al cerrar, no se hereda en silencio del login (2026-09-24)

- **Comportamiento anterior eliminado**: cuando un Mesero (no admin/telefonista) abría una mesa, `assigned_waiter` se guardaba automáticamente como el usuario logueado (`tables/views.py: table_open`, sigue igual — sigue haciendo falta algún valor al abrir, el modelo lo exige). El problema estaba en el cierre: `tables/services.py: close_table_account` tenía una sola línea, `account.tip_recipient = account.assigned_waiter`, que copiaba en automático quien fuera el responsable en ese momento como acreedor de la propina, sin confirmar nada. Esa línea se eliminó.
- **Comportamiento nuevo**: el diálogo "Cobrar {mesa}" ahora tiene un paso 4 obligatorio, "¿A nombre de quién se queda la mesa?", con botones por mesero (mismo listado que ya usaba "Cambiar mesero responsable"). No se puede enviar el formulario sin elegir (validado en JS con un mensaje de error visible, y en el formulario de Django con `responsible_waiter` como campo requerido). Al confirmar, `close_table_account` actualiza **tanto** `assigned_waiter` como `tip_recipient` con esa elección explícita.
- **Auto-impresión del ticket de cobro al cerrar**: justo después de un cierre exitoso, `tables/views.py: table_close` manda a imprimir el ticket de cobro en automático (`queue_ticket(..., ticket_type="payment", ...)`), ya con el responsable final elegido (no antes). Si la Dell no está disponible, el cierre **no se revierte** (ya está pagado); sólo se avisa con un mensaje. El estado del envío (enviado/fallido/expirado) se muestra dentro de la misma pantalla de resumen (`.closed-payment-summary`, elemento `[data-print-job-status]`), no como aviso flotante — se pasa el `job_id` por query string (`?printed_job=`) al redirigir, y `static/js/table-close.js` (`?v=8`) lo consulta contra `/app/impresion/estado/<id>/` cada 2s igual que el interceptor de impresión manual, pero actualizando ese elemento en vez del aviso flotante genérico.
- **Reasignar el responsable ahora funciona también con la cuenta ya cerrada** (antes `tables/services.py: reassign_table_account` rechazaba cualquier intento con `if account.status != OPEN`, y la plantilla ni mostraba el formulario si estaba cerrada). Tanto Mesero como Administrador pueden editarlo desde el resumen del ticket en cualquier momento.
- **Importante, pedido explícito del desarrollador con un escenario concreto**: si se reasigna el responsable de una cuenta **ya cerrada y ya vinculada** a un registro de Caja → Terminales (Clover/Mercado Pago/Transferencia), ese `TerminalMovement.tip_recipient` debe actualizarse también — si no, la Conciliación de propinas por persona y el reporte de propinas seguirían mostrando a la persona vieja aunque la mesa ya diga otra. `reassign_table_account` ahora hace `account.terminal_movements.update(tip_recipient=assigned_waiter)` (vía el `related_name` inverso, sin necesidad de importar `TerminalMovement` desde `orders` en `tables/services.py` y evitando así cualquier dependencia circular entre apps) cada vez que se reasigna una cuenta cerrada.
- Cambios: `tables/forms.py` (`TableAccountCloseForm.responsible_waiter`, nuevo campo requerido), `tables/services.py` (`close_table_account`, `reassign_table_account`), `tables/views.py` (`table_close` con auto-impresión, `table_detail` pasa `waiters` sin condicionar a `can_reassign` y agrega `auto_print_job_id`), `tables/templates/tables/table_detail.html` (paso 4 del diálogo de cierre, formulario de reasignación visible aunque esté cerrada, aviso inline de impresión), `static/js/table-close.js` (`?v=8`, botones de responsable + validación + polling de impresión).
- **Pruebas**: se corrigió también `tables/tests.py::TableInventoryIntegrationTests` (llamaba a `close_table_account` a mano sin `responsible_waiter`, y su usuario de prueba no tenía el grupo Mesero, lo cual ahora se valida). Suite completa de `tables` en verde (4 pruebas) y suite completa del proyecto en verde (90 pruebas) antes de este cambio.

## Corrección: liberar de Caja un pedido de Recoger en Efectivo sin definir monto ya no deja el pago sin definir (2026-09-24)

- Reportado por el desarrollador probando en local: en `/app/caja/`, si seleccionaba Efectivo en un pedido de Recoger pero nunca elegía billete/otra cantidad/pago exacto, y aun así le daba clic a "Liberar de caja", el pedido se liberaba con el monto pagado sin definir (`cash_tendered=None`) en vez de asumir pago exacto.
- Causa: `orders/services.py: set_cashier_release` sólo validaba que hubiera un `payment_method` elegido antes de liberar; nunca revisaba si, para Efectivo, ya se había definido cuánto pagó.
- Arreglo: si el pedido es de **Recoger** (no Entrega — las entregas tienen su propio flujo de cambio/propina) y el método es **Efectivo** y `cash_tendered` sigue sin definir al momento de liberar, se asume pago exacto (`cash_tendered = order.total`, `needs_change = False`) automáticamente, antes de completar el flujo de liberación. Si ya había un monto definido (por billete, otra cantidad, o pago exacto explícito), no se toca.
- **Pruebas nuevas** (`orders/tests.py`, clase `CashierReleaseCashDefaultTests`, 3 pruebas): liberar con Efectivo sin monto definido asume pago exacto; liberar con un monto ya definido no lo sobreescribe; liberar con un método distinto a Efectivo no toca `cash_tendered`/`needs_change`. Suite completa verificada sin fallos.

## Extendida la protección de conciliación de propinas a Entregas/Mercado Pago (además de Mesas/Clover) (2026-09-24)

- El desarrollador preguntó explícitamente si la protección que se le dio a Mesas (sincronizar `TerminalMovement` al reasignar responsable) también existía para pedidos de Entrega a domicilio (Mercado Pago). **No existía.** Se agregó el mismo patrón:
  - `orders/services.py: update_delivery_tip` — cuando Admin corrige la propina de una entrega (monto y/o destinatario, ya que `delivery_tip_recipient` sigue a `delivery_person`), ahora sincroniza `total_amount`, `tip_amount` y `tip_recipient` del `TerminalMovement` ya vinculado a ese pedido, si lo hay (`order.terminal_movements.update(...)`).
  - `orders/services.py: assign_delivery` — al reasignar el repartidor de una entrega que ya tenía propina registrada, además de actualizar `order.delivery_tip_recipient` (esto ya lo hacía), ahora también sincroniza el `tip_recipient` del `TerminalMovement` vinculado.
  - Si el pedido **todavía no está vinculado**, no hace falta nada más: `cashier_terminal_board` arma `link_candidates` en cada carga con los valores actuales del pedido, así que la próxima vez que se abra esa pantalla ya se ve todo actualizado.
- **Bug encontrado de paso, no relacionado con lo anterior pero en el mismo flujo**: en `/app/caja/`, la ficha de "Propina" para entregas ya tenía el comentario "la propina administrada sólo existe con Terminal o Transferencia" pero el código sólo la mostraba (`hidden`) para Transferencia — Terminal quedaba oculta por error, tanto en la carga inicial (`orders/templates/orders/includes/cashier_payment.html`) como al cambiar el método de pago con los botones (`static/js/cashier-board.js`, tenía el mismo `=== "transfer"` sin `|| "card"`). Se corrigieron ambos. `cashier-board.js` quedó en `?v=24`.
- **Escenario de prueba explícito del desarrollador** (pedido de $100 de consumo + $20 de propina para repartidor1, ya vinculado en Mercado Pago; luego Admin corrige a $10 de propina para repartidor4): cubierto con una prueba dedicada que corrige la propina y reasigna el repartidor juntos y verifica que el `TerminalMovement` termine con `tip_amount=10`, `total_amount=110`, `tip_recipient=repartidor4`.
- **Pruebas nuevas** (`orders/tests.py`, clase `DeliveryTipTerminalMovementSyncTests`, 3 pruebas).

## Corrección: "pago exacto" automático para Recoger + Efectivo también cubre avanzar el estado desde /app/pedidos/, no sólo Liberar de caja (2026-09-24)

- El arreglo anterior (ver arriba, "liberar de Caja...") sólo vivía dentro de `set_cashier_release`. El desarrollador señaló que también hay que cubrir el otro camino para llegar a "Recogido": avanzar el estado directamente desde el ticket en `/app/pedidos/` (el botón de siguiente acción, que llama a `transition_order` sin pasar por Caja).
- Además, investigando se encontró que `transition_order` ya tenía una validación para `complete_pickup` que sólo bloqueaba el avance si `order.needs_change` era `True` y el efectivo no alcanzaba — pero `needs_change` es `False` por default, así que un pedido en Efectivo al que nunca se le definió nada (ambos campos en su valor por defecto) pasaba esa validación sin problema y quedaba "Recogido" con `cash_tendered` sin definir. Mismo bug de fondo, dos entradas distintas.
- **Arreglo centralizado**: se movió la lógica de "asumir pago exacto si nunca se definió nada" de `set_cashier_release` a `transition_order` mismo, dentro del bloque `action == "complete_pickup"` — así cubre los dos caminos a la vez (Liberar de caja y avanzar directo desde Pedidos) sin duplicar código, y se borró el bloque que ya no hacía falta en `set_cashier_release` para no dejar código repetido sin usar.
- **Pruebas nuevas**: `test_advancing_status_directly_from_pedidos_also_assumes_exact_payment` en `orders/tests.py::CashierReleaseCashDefaultTests`, llamando a `transition_order` directamente (sin pasar por `set_cashier_release`) para confirmar que también asume pago exacto.

## Corrección: los vínculos que fallan por "ya se usó" dejaban la fila de Terminales a medias, y candidatos de otra pestaña se seguían ofreciendo (2026-09-24)

- Reportado probando en local: al intentar vincular un pedido/mesa que ya estaba tomado, la fila se quedaba con el Total/Propina/Propina-para del candidato fallido pegados, sin guardar nada real — "se veía trabada". `applyCandidateToRow` ahora guarda una foto de los valores previos antes de aplicar un candidato, y si el guardado falla con `link_already_used`, se restaura esa foto completa (no sólo el campo de vínculo).
- Causa real de fondo de por qué seguía fallando una y otra vez: `renderCandidateList` sólo revisaba las filas visibles **en la pestaña actual** para decidir qué estaba tomado — nunca miraba el `movement_id` que el servidor ya manda con cada candidato. Un pedido ya vinculado bajo OTRO proveedor (por ejemplo, vinculado en Mercado Pago antes de cambiarle el método de pago a Transferencia) nunca se detectaba como tomado en la pestaña de Transferencias, así que se seguía ofreciendo y el guardado fallaba siempre con el mismo 409, sin forma de resolverlo desde ahí. Ahora se revisan ambas cosas: el `movement_id` ya persistido (cualquier proveedor) y las filas de esta misma página con un vínculo optimista aún sin confirmar.
- **Nota para el futuro**: cambiar el método de pago de un pedido/mesa después de vincularlo NO libera el vínculo viejo automáticamente — eso no es un bug, simplemente no existe esa función todavía. Para reutilizar ese pedido en el proveedor correcto hay que desvincularlo primero desde la pestaña donde quedó atrapado.

## Corrección: aviso de "falta responsable de la mesa" no se veía al cerrar sin elegir mesero (2026-09-24)

- El `<select name="responsible_waiter">` vive dentro de un contenedor con `hidden` (los botones son la interfaz real). Django le agregaba el atributo HTML `required` automáticamente por ser un campo obligatorio; un `<select required>` oculto deja la validación nativa del navegador en un estado roto — puede bloquear el envío sin ningún aviso visible, porque no hay dónde anclar la burbuja de validación de un elemento sin caja visible.
- Arreglo: `tables/forms.py` agrega `HiddenRequiredSelect` (un `forms.Select` que sobreescribe `use_required_attribute` para que devuelva `False`) — el campo sigue siendo obligatorio tanto en Django como en la validación de JavaScript del formulario (que sí siempre muestra un aviso visible), pero el navegador ya no intenta validarlo por su cuenta. Verificado con Playwright temporal contra un navegador real: el aviso se muestra correctamente.

## Corrección: la columna "#" de Caja > Terminales no se actualizaba al agregar un registro nuevo (2026-09-24)

- La numeración sólo se calculaba en el servidor (`forloop.counter`) al cargar la página; un movimiento nuevo guardado por AJAX se quedaba con la celda de número vacía indefinidamente. `static/js/cashier-terminal-board.js` ahora renumera las filas guardadas en el cliente cada vez que se agrega o se borra un registro.

## Agregado: dividir cuenta en Mesas (2026-09-24)

Diseño confirmado con el desarrollador antes de programar: se bloquea hasta repartir el 100% de los artículos; un solo mesero se acredita como responsable en todas las cuentas divididas (sólo cambian método de pago/propina/efectivo por cuenta); un artículo no se puede repartir parcialmente entre dos cuentas; el número de cuentas sí se puede cambiar a media captura, y "Regresar" reinicia todo el proceso.

- **Modelo**: cada cuenta dividida es su propia `TableAccount`, todas apuntando a la misma mesa física. La cuenta que ya estaba abierta se convierte en la primera división; por cada división extra se crea una `TableAccount` nueva (mismo `table`, mismo `assigned_waiter`=responsable elegido). Los `TableAccountItem` se reparten cambiando su FK `account` a la cuenta que les toque. Cada una se cierra de forma independiente reutilizando la misma validación/snapshot de siempre — se extrajo esa lógica de `close_table_account` a un helper compartido `_finalize_account_close` para no duplicarla. Todo o nada dentro de una sola transacción: si una sola división falla su validación, ninguna se cierra. La mesa se libera sola en cuanto la última división queda cerrada (no hizo falta tocar esa regla, ya funciona así con múltiples cuentas cerradas por mesa).
- **Servicio nuevo**: `tables/services.py: split_and_close_table_account(*, account, splits, responsible_waiter, closed_by)`.
- **Vista/endpoint nuevo**: `tables/views.py: table_split_close` (`POST /app/mesas/cuentas/<id>/dividir/`) — recibe JSON (no un `<form>` normal, porque el número de cuentas y artículos por cuenta es variable) con `{"responsible_waiter": id, "splits": [{"item_ids": [...], "payment_method": ..., "tip_amount": ..., "cash_tendered": ...}, ...]}`. Manda a imprimir un ticket de cobro por cada cuenta cerrada (todas comparten el nombre de mesa automáticamente, ya que apuntan al mismo `table`) y redirige al mapa de mesas con un mensaje de éxito.
- **Interfaz**: un `<dialog>` nuevo y grande (`.table-split-dialog`), completamente separado del diálogo normal de cobro a propósito — así su CSS no pelea con ese layout ya delicado. Se abre con un botón "Dividir cuenta" dentro del paso 1 (Método de pago) del diálogo normal; "Regresar" cierra el panel de división, reinicia todo su estado, y vuelve a abrir el diálogo normal. Dentro: selector de cuántas cuentas, lista de artículos con botones 1/2/3... para asignarlos, una tarjeta por cuenta con su propio método de pago/efectivo/propina y total en vivo, y una sola selección de responsable compartida por todas. Reutiliza las clases de botones ya existentes (`.cashier-payment-buttons`, `.cash-quick-buttons`, `.tip-quick-buttons`, `.cash-selection-error`) en vez de duplicar estilos.
- Archivo nuevo: `static/js/table-split.js`.
- **Pruebas nuevas** (`tables/tests.py::TableSplitCloseTests`, 6 pruebas): el servicio cierra dos cuentas compartiendo la mesa con datos independientes; rechaza artículos sin asignar; rechaza un artículo en dos cuentas a la vez; la vista cierra la mesa y redirige al mapa; la vista exige un responsable; la vista exige al menos dos divisiones.
- Verificado visualmente con Playwright temporal (instalado, probado, desinstalado): flujo completo de abrir, dividir en 3, asignar artículos, elegir método de pago y efectivo exacto por cuenta, elegir responsable, y ver el aviso de validación cuando una cuenta se queda sin artículos — todo legible y funcionando.
- **Pendiente/fuera de alcance de esta primera versión**: no se construyó una vista de estado de impresión en vivo para los N tickets de una división (a diferencia del cierre normal, que sí muestra el estado del ticket de cobro en la pantalla de resumen) — se manda a imprimir cada uno, pero el usuario no ve confirmación en pantalla de cuáles se imprimieron. Si hace falta, sería un cambio aparte.

## Corrección: Dividir cuenta no dejaba asignar artículos — la lista se congelaba desde la carga de la página (2026-09-24)

- Reportado probando en local: el panel de Dividir cuenta se abría sin ningún artículo listado entre "Asigna cada artículo a una cuenta" y las tarjetas de cuenta.
- Causa: la primera versión armaba esa lista en el servidor, una sola vez, al cargar `/app/mesas/cuentas/<id>/`. Pero el resto de Mesas agrega/quita artículos por AJAX sin recargar la página (`table-pos.js: renderTicket`) — si la cuenta tenía 0 artículos al momento de cargar la página (o si se agregaron artículos después), el panel de dividir se quedaba con esa foto vieja, hasta vacía.
- Arreglo: `table-pos.js` ahora expone `window.__tableTicketItems` (inicializado desde un `json_script` embebido en la carga, `{{ ticket.items|json_script:"table-ticket-items-data" }}`, y actualizado cada vez que `renderTicket()` vuelve a dibujar el ticket). El panel de dividir (`table-split.js`) ya no depende de HTML renderizado por el servidor para su lista de artículos — la reconstruye desde `window.__tableTicketItems` cada vez que se abre, así que siempre refleja el ticket real, incluso si se acaban de agregar o quitar artículos sin recargar.
- **Pruebas**: se actualizó `tables/tests.py::TableSplitCloseTests.test_split_dialog_item_data_has_real_item_ids` para verificar el `json_script` en vez de atributos HTML estáticos (que ya no existen con este diseño).
- Verificado con Playwright temporal simulando exactamente el escenario reportado: cargar la página, agregar un producto por AJAX (sin recargar), abrir Dividir cuenta, y confirmar que el artículo recién agregado aparece en la lista — funcionó correctamente. El producto de prueba agregado a la cuenta real del desarrollador durante la verificación se eliminó después, junto con su movimiento de inventario huérfano.

## Nota: a partir de ahora el desarrollador lleva el `git commit`/`git push` (2026-09-24)

El desarrollador pidió explícitamente que, de aquí en adelante, él se encargue de subir los cambios a git — Claude sigue haciendo y probando los cambios (incluyendo correr la suite de pruebas), pero ya no ejecuta `git add`/`commit`/`push` por su cuenta salvo que se le pida.

## Agregado: hora/fecha de entrega y nombre del cliente en negritas en la comanda de cocina, sólo para Entrega/Recoger (2026-09-24)

- Pedido del desarrollador: en la comanda de cocina (no en Mesas), para pedidos de Entrega a domicilio o Recoger, agregar la hora y fecha de entrega solicitada, y que el nombre del cliente se vea tan marcado (negritas) como ya se ve la etiqueta "A DOMICILIO"/"RECOGER" del encabezado.
- `config/printing.py: order_print_context` ya traía `requested_for` en el contexto (no se usaba en esta plantilla) — sólo hizo falta mostrarlo en `templates/printing/kitchen_ticket.html`, dentro de la rama que no es Mesas (`source_kind != 'table'`), y sólo si el pedido tiene una hora solicitada (`requested_for` puede ser nulo para pedidos sin hora específica, en cuyo caso el renglón simplemente no aparece). También se envolvió el valor del nombre del cliente en `<strong>` (antes sólo la etiqueta "Cliente:" estaba en negritas, no el nombre en sí).
- Como `order_print_context` alimenta las 4 rutas de impresión de cocina de pedidos (vista previa manual, "modificado", y la cola automática de la Dell), el cambio aplica en todas por igual sin tocar nada más.
- **Pruebas nuevas** (`orders/tests.py::KitchenTicketPrintTests`, 2 pruebas): un pedido de entrega con `requested_for` muestra el renglón "Entrega:" y el nombre en negritas doble; un pedido de recoger sin `requested_for` no muestra ese renglón.

## Corrección: "Bolillo" (y cualquier producto suelto de "Comida por orden") no aparecía en el buscador en horario de desayunos, ni en Mesas ni en Pedidos (2026-09-24)

- Reportado: buscar "bolillo" para agregarlo suelto no encontraba nada, tanto en Mesas como en Pedidos, aunque el producto está correctamente configurado (`is_available`, `is_sold_individually`, con periodos de servicio "Desayuno" y "Comida" ambos activos).
- Causa raíz: tanto `tables/views.py` (`table_detail`) como `orders/views.py` (`internal_order_edit`/`internal_order_create`) envolvían el cálculo COMPLETO de `daily_order_loose_products` (los productos sueltos de la categoría "Comida por orden", como Bolillo) en un candado de todo-o-nada `if capture_mode == 'lunch'` / `if mode == 'lunch'`. Como el modo automático es "desayuno" de 7:00 a 13:00, cualquier producto suelto — sin importar su propia configuración — desaparecía por completo del catálogo (y por lo tanto del buscador) durante toda la mañana.
- Se consideró usar `product_is_available_now(..., enforce_service_period=True)` para respetar el periodo de servicio de cada producto individualmente, pero se descartó: el texto de ayuda del propio admin (`menu/forms.py`, campo `service_periods`) documenta explícitamente que "Mesas ignora el horario, pero el menú público sí lo aplica" — los periodos de servicio son sólo para el menú público (`menu/views.py: public_menu`), nunca para la captura interna de Mesas/Pedidos. Forzar el periodo aquí habría sido una regresión real: bloquearía a los meseros de vender Bolillo antes de las 8am o después de las 5pm, algo que hoy sí pueden hacer con cualquier otro producto.
- **Arreglo real**: se quitó por completo el candado de modo alrededor de `daily_order_loose_products` en ambas vistas — ahora siempre se calcula, sin importar el modo de captura, dejando que cada producto se filtre únicamente por su propia configuración normal (`is_available`, `is_sold_individually`, categoría, existencias del día).
- En Mesas esto bastaba: el buscador (`catalog_search_products`, sección `data-catalog-search-results`) ya mezclaba `daily_order_loose_products` sin depender del modo, así que sólo hacía falta que la lista dejara de estar vacía en modo desayuno.
- En Pedidos hizo falta un segundo ajuste: el buscador ahí no lee JSON en vivo, arma sus resultados clonando tarjetas `[data-product-name]` ya presentes en el HTML renderizado por el servidor (`static/js/internal-order-form.js: sourceCards`). La sección `#internal-daily-orders` (y su botón de navegación "Comida por orden") estaban también envueltos en `{% if menu_mode == 'lunch' %}` en la plantilla `orders/templates/orders/internal_order_form.html`, así que aunque la vista ya calculara la lista, la tarjeta de Bolillo nunca llegaba al HTML en modo desayuno. Se sacó esa sección (y su botón) del candado de modo, dejando "Comida corrida"/"Comida ejecutiva" como las únicas secciones exclusivas de modo comida (siguen dependiendo del menú diario publicado, que sí es de mediodía).
- **Pruebas nuevas**: `tables/tests.py::TableLooseProductVisibilityTests` (2 pruebas, confirman que Bolillo aparece en `daily_order_loose_products` tanto en modo comida como en modo desayuno) y `orders/tests.py::InternalOrderReservedCategoryTests.test_loose_product_is_also_visible_in_breakfast_mode` (confirma que la tarjeta con `data-product-name="bolillo"` se renderiza también en modo desayuno).

## Robustez: reintento del mismo ticket cuando Edge muere a media renderización, en el agente de la Dell (2026-09-24)

- Reportado: errores intermitentes de impresión con el texto `BrowserType.launch: Target page, context or browser has been closed` (ya documentado en el propio `agent.py`: ocurre cuando el Edge headless que usa Playwright para dibujar el ticket se cierra solo — crash, falta de memoria, Windows lo mata, etc.).
- El agente ya tenía una defensa parcial: antes de CADA trabajo comprueba `browser_alive()` y reabre Edge si ya estaba muerto de un trabajo anterior. Pero no cubría el caso de que Edge muriera *durante* la renderización del ticket actual (ya vivo al empezar, pero se cae en `render_ticket`) — ese caso se iba directo a `failed` sin reintentar, obligando a reimprimir a mano.
- **Cambio**: `print_station/dell_agent/agent.py` — si `render_ticket` lanza una excepción, se reabre Edge una vez más y se reintenta el mismo ticket antes de marcarlo como fallido. Sólo si ese segundo intento también falla, el trabajo se reporta como `failed` al servidor (igual que antes).
- Esto reduce cuántos tickets terminan fallando por un tropiezo puntual de Edge, pero no elimina la causa de fondo de por qué Edge se cae de vez en cuando (memoria, actualización automática de Edge, procesos zombie, etc.) — si sigue ocurriendo seguido después de este cambio, hace falta revisar eso directamente en la Dell.
- **Importante para desplegar**: este archivo corre localmente en la Dell (no en Lightsail), así que el cambio no queda activo hasta que se actualice esa copia en la Dell y se reinicie el proceso/tarea programada del agente.
- Sin pruebas automatizadas (el script depende de `pywin32`/Playwright con Edge real, fuera del entorno de pruebas de Django).

## Corrección: "Imprimir cocina modificado" no avisaba si el trabajo se imprimió, y el aviso de éxito en Mesas no salía en verde (2026-09-24)

- Reportado: en Pedidos, después de enviar una comanda con "Imprimir cocina modificado" (la variante que deja elegir partidas y cantidades sueltas), no aparecía ningún aviso de si la Dell realmente la imprimió o falló — a diferencia del cierre de cuenta en Mesas, que sí muestra ese aviso. Además, en Mesas, cuando el ticket de cobro SÍ se imprime bien, el aviso aparece en texto normal (no en verde); sólo el aviso en rojo (cuando falla) se ve con su color.
- **Causa 1 (Pedidos sin aviso)**: `orders/views.py: order_kitchen_custom_print` mandaba el trabajo a la cola (`queue_ticket`) y redirigía a `order_detail` sólo con un mensaje flash ("Comanda enviada a la Dell..."), sin pasar el id del trabajo ni tener ningún elemento en la plantilla que consultara `/app/impresion/estado/<job_id>/`. Mesas sí tenía ese mecanismo completo (`auto_print_job_id` en el contexto, `[data-print-job-status]` en la plantilla, y el sondeo en `table-close.js`) pero nunca se replicó para esta pantalla.
- **Arreglo**: se copió el mismo patrón a Pedidos. `order_kitchen_custom_print` ahora redirige con `?printed_job=<job.pk>`; `order_detail` lee ese parámetro igual que `table_detail`; `orders/templates/orders/order_detail.html` agrega el mismo `<p data-print-job-status data-job-id="...">`; y se agregó el bloque de sondeo (idéntico al de `table-close.js`, con los textos ajustados a "comanda") al final de `static/js/order-detail.js`.
- **Causa 2 (verde faltante en Mesas)**: en `table-close.js`, la rama de éxito (`job.status === "printed"`) sólo cambiaba el texto del aviso; nunca le agregaba una clase de color, a diferencia de las ramas de error que sí agregan `text-danger`. Tampoco existía una clase `.text-success` en `app.css` — sólo `.text-danger`.
- **Arreglo**: se agregó `printStatus.classList.add("text-success")` en la rama de éxito de `table-close.js` (y en la nueva rama equivalente de `order-detail.js`), y se agregó `.text-success { color: #285943 !important; font-weight: 800; }` en `app.css`, junto a `.text-danger`, usando el mismo verde de marca que ya se usa en botones y encabezados.
- **Pruebas nuevas** (`orders/tests.py::KitchenCustomPrintStatusTests`, 2 pruebas, requieren simular la estación de impresión en línea con `PrintStation.objects.create(printer_available=True, last_heartbeat_at=...)` y `PRINT_AGENT_TOKEN`): confirman que encolar una comanda modificada redirige con `?printed_job=`, y que `order_detail` renderiza el marcador `data-print-job-status` para ese trabajo.

## Diagnóstico en producción: dos agentes de la Dell corriendo a la vez explicaban el "a veces sí, a veces no" (2026-09-24)

- El desarrollador seguía viendo `BrowserType.launch: Target page, context or browser has been closed` de forma intermitente en el admin, incluso después del arreglo de reintento del mismo día. Se descartó primero la hipótesis del incidente del 22 de septiembre (Tarea Programada corriendo como `SYSTEM`, sin sesión de escritorio real) — `Get-ScheduledTask ... | Select Principal` confirmó `LogonType: Interactive`, `UserId: lleva`, no `SYSTEM`.
- `Get-Content agent.log -Tail 40` mostró que el log llevaba **dos días sin actualizarse** (última entrada del 22 de septiembre, y ni siquiera del error de Edge — era un corte de red viejo). Eso fue la pista real: lo que estuviera imprimiendo en ese momento no estaba escribiendo a ese archivo.
- `Get-Process python | Select Id, StartTime, Path` reveló la causa real: **dos instancias de `agent.py` corriendo al mismo tiempo** — una arrancada el 23 de septiembre a las 9:35 a.m. (llevaba más de un día viva) y otra arrancada el 24 a las 11:32 a.m., probablemente iniciada a mano encima de la anterior sin que nadie se diera cuenta de que ya había una corriendo. (Cada arranque de `agent.py` se ve como un par de PIDs porque el `.venv` en Windows usa un proceso "redirector" hacia el intérprete real — eso es normal; lo anormal era tener dos pares.)
- Dos agentes compitiendo por la misma cola y la misma POS-80 significa dos Edge headless abiertos a la vez en la misma Dell, compitiendo por memoria/CPU — encaja perfectamente con un fallo intermitente en vez de total: dependía de cuál de los dos procesos le tocara el siguiente trabajo.
- **Arreglo aplicado en la Dell** (comandos de PowerShell entregados al desarrollador, quien los ejecutó): se mataron todos los procesos `python.exe` cuya línea de comando incluyera `dell_agent` (`Get-CimInstance Win32_Process -Filter "Name='python.exe'" | Where CommandLine -like '*dell_agent*' | Stop-Process`), se reescribió `agent.py` completo en la Dell con la versión del reintento (vía `Set-Content -Encoding UTF8` con un here-string de PowerShell, ya que esa carpeta no es un clon de git — ver nota del 19 de septiembre), y se relanzó con `Start-ScheduledTask`. Verificado después: sólo un par de PIDs, ambos con `StartTime` recién arrancado.
- **Nota para el futuro**: si vuelve a reportarse un fallo intermitente de impresión, revisar primero `Get-Process python -ErrorAction SilentlyContinue | Select Id, StartTime, Path` en la Dell — más de un par de PIDs es la señal inequívoca de agentes duplicados peleando por la misma impresora.
- Pendiente de confirmar por el desarrollador si, con el duplicado eliminado, el error de Edge deja de aparecer o sólo se reduce en frecuencia (en cuyo caso el reintento del mismo día seguiría cubriendo los casos residuales).

## Incidente de impresión (continuación, mismo día): el agente sólo sobrevivía mientras alguien lo corría a mano dentro de una sesión SSH (2026-09-24)

- Después del arreglo de duplicados, el desarrollador reportó "ya no imprime" de urgencia. Diagnóstico: durante todo este proceso, los comandos en "la Dell" en realidad se corrían dentro de una sesión **SSH** hacia la Dell, abierta desde la laptop personal del desarrollador. Cuando esa laptop se bloqueaba, la conexión SSH se cortaba, y Windows mataba el proceso `agent.py` que corría en primer plano dentro de esa sesión junto con ella — apagando la impresión para todo el negocio sin ningún aviso visible del lado del desarrollador (su laptop sólo se veía "bloqueada").
- Se recuperó la impresión de inmediato corriendo `agent.py` manualmente otra vez (parche temporal, con la advertencia explícita de no bloquear la laptop mientras tanto).
- Se intentó la solución real (`Start-ScheduledTask "SuperCocina Print Agent"`, pensada para correr independiente de cualquier SSH) y no arrancó nada. `query session` confirmó que sí existe una sesión de consola activa de `lleva` (descartando el problema de sesión-interactiva-inexistente), así que la causa era otra.
- Causa real encontrada: `Get-ScheduledTask ... | Select Actions` mostró que la Acción de la tarea apuntaba **directo** a `python.exe agent.py`, saltándose por completo el `run_agent_hidden.vbs`/`run_agent.bat` que existían en disco (correctamente escritos) pero que la tarea ya no invocaba — la configuración se había desviado otra vez de lo documentado (ya van varias veces, ver incidentes anteriores de este mismo archivo). Por eso `agent.log` nunca mostraba nada nuevo: ese camino nunca pasaba por la redirección que escribe el log.
- Se corrigió la Acción de la tarea con `Set-ScheduledTaskAction`/`New-ScheduledTaskAction` para que vuelva a apuntar a `wscript.exe run_agent_hidden.vbs`. **Lección aprendida en el camino**: mientras se probaba, se dejaron corriendo el agente manual Y el de la tarea al mismo tiempo por unos segundos para comparar — eso bastó para que, poco después, AMBOS terminaran muriendo (parece que dos Edge headless compitiendo de nuevo). Recomendación para el futuro: nunca dejar dos agentes corriendo a la vez, ni para pruebas cortas — matar uno por completo antes de levantar el otro.
- **Verificado al final**: con la Acción corregida, matando el agente manual primero y levantando sólo el de la Tarea Programada después (sin duplicarlos), el par de procesos se mantuvo vivo después de que el desarrollador cerró por completo su sesión SSH — confirmando que ya no depende de mantener una conexión abierta.
- **Lección para futuras sesiones de soporte de este componente** (guardada también en memoria): nunca tratar "correr `agent.py` en primer plano dentro de una sesión SSH" como solución permanente — sólo como parche de emergencia, avisando explícitamente que hay que mantener esa sesión y esa laptop despiertas mientras tanto. La solución real siempre es que la Tarea Programada logre arrancarlo sola, y la única prueba válida de que quedó bien es que el proceso siga vivo después de cerrar el SSH por completo.

## Tres bugs de UI encontrados y corregidos en la misma sesión (2026-09-24)

- **Repartos: la lupa de búsqueda no abría el teclado ni dejaba escribir.** Causa: condición de carrera entre `static/js/delivery-board.js` (que abre la búsqueda al tocar la lupa) y el listener global de `static/js/search-dismiss.js` — este último, al ver el clic en el mismo botón de lupa, encontraba el input de búsqueda dentro del mismo `<form>` (aunque siguiera oculto) y disparaba su lógica de cierre, que terminaba haciendo clic sintético sobre el fondo oscuro recién mostrado y cancelando la apertura antes de que el `.focus()` real llegara a ejecutarse. Arreglo: `search-dismiss.js` ahora ignora por completo los clics sobre los botones que ABREN una búsqueda (`[data-menu-search-open]`, `[data-delivery-search-open]`, `.compact-field-launcher.is-product-search`) cuando la búsqueda todavía no está abierta, dejando que cada pantalla maneje su propia apertura; sigue cerrando correctamente cuando se toca la lupa por segunda vez con la búsqueda ya abierta.
- **Caja y Repartos: el campo de búsqueda de texto sólo dejaba escribir un carácter a la vez.** Causa: ambos formularios (`cashier-board.js`, `delivery-board.js`) tenían una búsqueda "en vivo" que, 450-500ms después de cada tecla, recargaba la página COMPLETA (`form.requestSubmit()` sobre un `<form method="get">` normal, no por AJAX). En móvil, la pausa natural entre letras suele superar esos 450-500ms, así que cada pausa disparaba una recarga real que quitaba el foco del campo — dando la sensación de que sólo dejaba escribir una letra. Arreglo: se quitó el auto-envío por cada tecla en ambos archivos; ahora el campo de texto sólo busca con Enter (envío nativo del formulario, sin necesitar botón) o cuando cambian fecha/selects/repartidor/estado (eso sí sigue auto-enviando, ya que no se escribe carácter por carácter).
- **Repartos: al abrir la búsqueda, se veía un recuadro con "Buscar reparto" en negritas pero sin ver lo que se escribía ni el cursor.** Diagnosticado con una sesión temporal de Playwright inspeccionando estilos computados (instalado, usado, desinstalado igual que otras veces esta sesión) porque no era evidente leyendo el CSS a simple vista: las reglas `.delivery-person-filter input/label/span` (pensadas sólo para ocultar visualmente los botones de radio "Todos/Sin asignar/repartidor" y mostrarlos como pastillas) alcanzaban también al input de búsqueda de texto, porque ambos viven anidados dentro de la misma sección `.delivery-person-filter` — el input de búsqueda terminaba con `opacity: 0` y encogido a `1px`, mientras el `<span>` de su etiqueta (pensado para ser una leyenda pequeña) se estiraba visualmente para parecer el cuadro completo. Arreglo: esas tres reglas (y sus variantes en modo oscuro y en el breakpoint móvil) se reescribieron para apuntar sólo a `.delivery-person-options` (el contenedor real de los radios), dejando intacto el input de búsqueda.

## Caja: se habilitó el botón de cambiar estado del pedido en las tarjetas de entrega a domicilio (2026-09-24)

- Pedido del desarrollador: usar el espacio que ya se esconde en tablet/celular dentro de las tarjetas de entrega de Caja (detrás del botón "ver más") para poder avanzar el estado del pedido directamente desde ahí, sin tener que abrir el pedido completo.
- Resultó que el control ya existía por completo: `orders/templates/orders/includes/cashier_status_action.html` (el mismo botón que ya usan las tarjetas de "Recoger en la fonda" en Caja) se incluía también en las tarjetas de entrega a domicilio, pero con `hide_action=True` — con una nota en el propio código diciendo que Caja lo apagaba "mientras se define el flujo". El botón de acción vive dentro de `.cashier-delivery-details`, que en tablet/celular ya está oculto por defecto y sólo aparece al expandir la tarjeta (`.is-detail-expanded`), que es justo donde el desarrollador pidió que apareciera.
- **Arreglo**: se quitó `hide_action=True` de esa inclusión en `orders/templates/orders/cashier_board.html`. La transición ofrecida ya depende del estado y del rol de quien la ve (`_order_actions_for_user`/`available_order_actions`, reutilizado de Pedidos) — para entregas ofrece "Despachar a reparto" en Listo y "Completar entrega" en En reparto, igual que ya sucede en la vista de Pedidos. No hizo falta tocar JavaScript ni CSS.
- **Pendiente/sin resolver todavía**: el desarrollador reportó por separado no poder reasignar el repartidor de un pedido "En reparto"/"Entregado" estando con una cuenta que dice ser Administrador — revisando el código, esa combinación específica (Admin + esos dos estados) no debería bloquearse (`_order_locked_for_edit` se salta explícitamente para Admin). Se le pidió confirmar que la cuenta usada de verdad tiene el rol Administrador asignado, y si aun así vuelve a pasar, el mensaje de error exacto que aparece — sigue sin reproducirse ni diagnosticarse a fondo.

## Cuatro bugs más reportados y corregidos el mismo día (2026-09-24)

- **Pedidos: el mismo bug de la lupa de Repartos/Caja (sólo dejaba escribir una letra) también estaba en el campo "Buscar" del listado de Pedidos.** Misma causa (`static/js/order-list.js` recargaba la página completa 500ms después de cada tecla) y mismo arreglo: se quitó el auto-envío por tecla; sólo busca con Enter o cambiando fecha/select.

- **Mesas: "Comida ejecutiva" no dejaba crear el paquete porque "Segundo tiempo" nunca se veía.** Diagnosticado con una sesión temporal de Playwright (instalada, usada y desinstalada) porque no era evidente a simple vista: "Segundo tiempo" SÍ existía en el DOM y estaba bien posicionado por CSS Grid (columna 1, fila 2) — pero "Agua del día" (parte de `.table-package-visual-extras`) también caía en esa MISMA celda, tapándolo por completo. Causa raíz: la regla base `.table-visual-package-dialog .package-visual-groups > .table-package-visual-extras .table-water-choice { grid-column: 1; grid-row: 2; }` está pensada para el layout de 3 columnas de "Comida corrida" y nunca se excluyó para el layout de 2 columnas de "Comida ejecutiva" (donde esa misma celda ya la usa Segundo tiempo). Ya existía un arreglo para móvil (`@media max-width: 900px`) que resolvía esto abriendo una tercera fila, pero nunca se agregó el equivalente para escritorio — el bug existía en cualquier ancho de pantalla normal, no sólo en celular. **Arreglo**: nueva regla, sólo para Mesas y sólo para `[data-package-kind="executive"]`, que abre una tercera fila para "Agua del día" y extiende "Tercer tiempo · plancha" para seguir ocupando toda la altura de la columna. Se dejó anotado (no corregido, por alcance) que el mismo bloque de estilos tiene una copia duplicada más arriba en el archivo (`static/css/app.css` líneas ~244 y ~3334), ya inactiva por completo gracias a un bloque "final" posterior — candidato a limpieza futura.

- **Mesas: los diálogos de "Comida corrida"/"Comida ejecutiva" se abrían mostrando marcada la comida seleccionada la vez anterior.** Causa: `form.reset()` (llamado al abrir el diálogo) sí limpia los radios reales, pero `reset()` nunca dispara el evento `change` — y los contadores (+/-) y el resaltado de cada tarjeta sólo se sincronizan escuchando ese evento (`static/js/package-selection.js`). El formulario quedaba limpio de verdad, pero la vista se quedaba pegada a la selección anterior. **Arreglo** en `static/js/table-pos.js`: después de `form.reset()` (al crear un paquete nuevo) y después de `setRadio(...)` (al editar uno existente), se disparan manualmente eventos `change` sobre los radios de cada tiempo para forzar la resincronización visual.

- **Mesas: Dividir cuenta no permitía repartir producto por producto cuando dos comidas idénticas quedaban agrupadas en una sola partida.** Causa raíz: cuando se agrega dos veces la misma combinación exacta (mismo paquete/producto/configuración), `add_package_to_table`/`add_product_to_table` **incrementan la `quantity` de la partida existente en vez de crear una fila nueva** — así que "2x Comida corrida" en el ticket es UNA sola fila de `TableAccountItem` con `quantity=2`, no dos filas separadas. El panel de dividir sólo podía mover la partida completa (por su único `pk`), nunca una unidad suelta.
  - **Arreglo de backend** (`tables/services.py: split_and_close_table_account`): el contrato de `splits` cambió de `{"items": [TableAccountItem, ...]}` a `{"item_quantities": {item_pk: cantidad, ...}}`. Ahora un mismo `item_pk` puede repartirse en varias divisiones, siempre que la suma de cantidades asignadas coincida exactamente con la `quantity` real de esa partida. Cuando una partida se reparte, todas las porciones menos la última se separan en filas nuevas (mismos datos clonados vía `pk = None` + `save()`, cantidad y subtotal recalculados); la porción restante se queda en la fila original, que termina asignándose completa a la última división que la usa — así el caso normal (sin dividir nada) sigue sin crear filas de más. **Limitación conocida y documentada en el propio código**: los movimientos de inventario (`StockMovement`) de la partida original se consumen todos juntos con la primera división que se cierra, no se reparten entre las partidas nuevas — el total de inventario sigue siendo correcto, sólo la atribución de "qué cuenta disparó el consumo" puede no ser exacta para las porciones separadas.
  - Como una mesa sólo puede tener una `TableAccount` abierta a la vez (restricción de base de datos `unique_open_account_per_table`), las cuentas nuevas de cada división se siguen creando y cerrando una por una, en orden — no se pueden crear todas por adelantado (se intentó primero así y falló con un `IntegrityError` en las pruebas).
  - **Vista** (`tables/views.py: table_split_close`): el JSON esperado por cuenta dividida cambió de `{"item_ids": [...]}` a `{"items": [{"item_id": ..., "quantity": ...}, ...]}`.
  - **Frontend** (`static/js/table-split.js`): el panel ahora dibuja una fila por CADA unidad de una partida agrupada (mismo `item_id`, tantas filas como `item.quantity`, con el subtotal dividido entre ellas y, si hay más de una, el nombre marcado "(1/2)", "(2/2)", etc.), reutilizando exactamente la misma interacción de "elegir cuenta 1/2/3..." que ya existía por fila. Al enviar, las filas asignadas se vuelven a agrupar por `item_id` contando cuántas unidades le tocaron a cada cuenta.
  - **Pruebas nuevas** (`tables/tests.py::TableSplitCloseTests`): `test_split_service_rejects_quantity_mismatch_for_an_item` (reemplaza a la prueba vieja de "un artículo no puede estar en dos cuentas", que ya no aplica — ahora SÍ puede, mientras las cantidades cuadren), `test_split_service_splits_a_grouped_item_unit_by_unit` y `test_split_view_splits_a_grouped_item_between_two_accounts` (confirman que una partida con `quantity=2` se puede repartir 1 y 1 entre dos cuentas, con los subtotales correctos).
  - Sin verificación visual con Playwright para este cambio en particular (el desarrollador prefiere probar la interfaz él mismo) — las 9 pruebas de `TableSplitCloseTests` (113 en total en todo el proyecto) sí quedaron corriendo en verde.

## Pedidos: nota general, guardar/cerrar e imprimir movidos arriba del ticket, en una cuadrícula de 3×2 (2026-09-24)

- Pedido del desarrollador, explícitamente marcado como "limpieza": mover "Guardar sin cerrar" y "Cerrar captura y enviar al listado" (exclusivos de Pedidos) hasta arriba del panel de ticket, junto con "Agregar nota general" e "Imprimir cocina/cobro/modificado", en una sola cuadrícula de 3 columnas × 2 filas (fila 1: nota, guardar, cerrar; fila 2: los tres de imprimir) — y eliminar el código viejo en vez de dejarlo muerto al lado del nuevo.
- **Antes**: "Agregar nota general" y los tres enlaces de imprimir se creaban con JavaScript (`static/js/internal-order-form.js`) y se insertaban por código justo debajo del encabezado del ticket; "Guardar sin cerrar"/"Cerrar captura..." vivían renderizados directamente en la plantilla, pero hasta ABAJO de todo (después de los artículos, el total y el resumen de pago) — un intento previo de moverlos arriba nunca se hizo, y el guardado/impresión se quedaron separados en dos puntos distintos del panel.
- **Ahora**: los 6 controles se dibujan directamente en la plantilla (`orders/templates/orders/internal_order_form.html`), ya en su posición final, dentro de un solo `<div class="internal-ticket-quick-actions">` justo debajo de `.current-ticket-heading`. Ya no hace falta que JavaScript los cree ni los reubique — sólo sigue leyendo los elementos existentes para mantener su estado sincronizado (el texto del botón de nota cambia entre "Agregar"/"Editar nota general", y los enlaces de imprimir se deshabilitan visualmente si el ticket se vacía).
- Se eliminó por completo del código: el `<span hidden data-order-note-config>` y el `<div hidden data-internal-print-urls>` (ya no hacen falta, los datos viven directo en los atributos de los botones/enlaces reales); el bloque de JavaScript que creaba y reubicaba esos elementos (~35 líneas); la clase `.internal-ticket-actions` y **13 bloques de CSS** repartidos por todo `app.css` que la usaban (varios ya duplicados entre sí, herencia de parches anteriores sin limpiar); las referencias a `.internal-live-ticket > .ticket-print-actions` (que dejaron de aplicar porque ese elemento ya no es hijo directo del ticket, sino que vive dentro de la nueva cuadrícula con `display: contents` para participar como si lo fuera).
- CSS nuevo, sólo para Pedidos: `.internal-ticket-quick-actions` (la cuadrícula 3×2) más un ajuste para pantallas angostas (`max-width: 640px`). Las reglas compartidas con Mesas (`.ticket-print-actions` base, `.table-pos-layout .table-ticket:not(.is-closed-summary) > .ticket-print-actions` y variantes) se dejaron intactas — Mesas nunca tuvo estos botones de guardar/cerrar y su impresión sigue exactamente igual que antes.
- **Prueba actualizada**: `orders/tests.py::CapturePrintTests.test_new_capture_exposes_print_actions_before_first_item` verificaba la presencia del ahora-eliminado `data-internal-print-urls`; se cambió para verificar `internal-ticket-quick-actions` en su lugar, conservando la comprobación real (los enlaces de imprimir siguen disponibles incluso antes de agregar el primer producto).
- Verificado visualmente con Playwright temporal (instalado, usado, desinstalado) en dos anchos (480px celular y 1400px escritorio): la cuadrícula se ve exactamente como se pidió en ambos casos, incluyendo el ajuste angosto de texto en celular para "Cerrar captura y enviar al listado".

## Confirmado: los dos bugs de Mesas reportados de nuevo eran caché del navegador, no un fallo real (2026-09-24)

- El desarrollador reportó que el bug de "Segundo tiempo" invisible en Comida ejecutiva y el de las selecciones que se quedan marcadas al reabrir seguían presentes — pero la captura de pantalla era de `supercocina.win` (producción), donde los arreglos de hoy todavía no se habían subido ni desplegado (el desarrollador lleva esa parte). Confirmó después que fue error suyo de caché al probar; ambos arreglos siguen vigentes tal como se documentaron arriba. Sin cambios de código en esta entrada — sólo aclaración para no repetir la confusión.

## Cambio de denominaciones de "efectivo recibido": se quitó el billete de $20 y se agregó el de $150, en todos los lugares donde aparecía (2026-09-24)

- Pedido del desarrollador: en TODOS los selectores rápidos de "¿con cuánto paga?"/"efectivo recibido" que incluían el billete de $20, cambiar el orden a $50, $100, $150, $200, $500 (y "Exacto" al final, donde ya existía como botón en la misma fila).
- Se encontraron y actualizaron 6 lugares distintos (server-rendered HTML, JS y validación de formularios Django — no sólo la interfaz):
  - `tables/templates/tables/table_detail.html` — "2. Efectivo recibido" del diálogo de cobro en Mesas.
  - `static/js/table-split.js` — tarjetas de cuenta del panel de Dividir cuenta (Mesas).
  - `orders/templates/orders/internal_order_form.html` — "¿Con cuánto paga?" en la captura interna de Pedidos.
  - `orders/templates/orders/includes/cashier_payment.html` — botones de efectivo en Caja para pedidos de entrega/recoger (éste ya tenía "Exacto" como sexto botón, se conservó al final).
  - `orders/templates/orders/delivery_board.html` (vía el contexto `cash_denominations` en `orders/views.py: delivery_board`) — "¿Con cuánto paga?" en Repartos, al cambiar el método de pago de una entrega.
  - `orders/forms.py` — los dos `ChoiceField` de `cash_bill` que validan la captura interna de Pedidos (formulario principal y el de autoguardado) tenían `$20` como única opción válida además de las demás; sin este cambio, el frontend habría podido ofrecer `$150` pero el servidor lo habría rechazado como opción inválida. También se actualizó `orders/views.py: _internal_order_initial` (la tupla que decide si un pedido ya guardado con cierto `cash_tendered` se preselecciona como "billete estándar" al reabrir la captura, o se trata como monto personalizado).
- **Pruebas**: `orders/tests.py::CapturePrintTests.test_print_autosave_rejects_insufficient_cash` usaba `cash_bill: "20"` para probar que un efectivo insuficiente se rechaza — como el pedido de prueba cuesta $38 y ya no existe un billete estándar por debajo de $50, se cambió a `cash_custom_amount: "20"` (un monto libre, no uno de los billetes fijos), que prueba exactamente lo mismo sin depender de qué denominaciones estén disponibles.
- No se tocó el formulario del carrito público (`orders/forms.py: PublicCheckoutForm`) porque ese selector, orientado al cliente final, nunca tuvo la opción de $20 para empezar.

## Botones de acción del ticket: más pequeños en tablet/escritorio, misma tipografía y centrado real, y en Mesas los de imprimir también suben arriba (2026-09-24)

- Alcance confirmado con el desarrollador antes de tocar CSS: sólo los botones de acción del ticket (nota general, guardar/cerrar, imprimir, cobrar/dividir cuenta) en Mesas y Pedidos — no cada botón de la aplicación (tarjetas de producto, método de pago, filtros, etc. quedan intactos).
- **Tipografía inconsistente, encontrada con Playwright temporal inspeccionando estilos computados** (no era evidente leyendo el CSS): el sitio ya tiene una regla deliberada que pone `<button>` en "Nunito Sans" para legibilidad ("Tipografía clara para controles y botones operativos"), pero los tres enlaces de imprimir (`<a>`, no `<button>`) quedaron fuera de esa lista desde siempre y heredaban "Poppins" (la fuente base del sitio) con un tamaño de letra mayor — por eso "Agregar nota general" se veía visualmente distinto/más legible que "Imprimir cocina" aunque ambos son parte del mismo grupo de botones. Se agregó `.ticket-print-actions a` a esa regla compartida.
- **Tamaño en tablet/escritorio**: la cuadrícula 3×2 de Pedidos (agregada el mismo día) sólo tenía un ajuste compacto para celular (`max-width: 640px`); arriba de ese ancho los botones se veían grandes, y los de imprimir en particular salían más altos que el resto de la fila por el `padding`/tamaño de letra heredado de `<a>`. Se unificó a un solo tamaño compacto para todos los anchos (ya no hace falta el `@media` aparte) y se le agregó el mismo tratamiento a los tres enlaces de imprimir dentro de la cuadrícula.
- **Centrado "literal"**: los botones usaban `text-align: center` pero seguían siendo `display: block`, así que un botón con una sola línea de texto dentro de una celda más alta que su contenido (por ejemplo, cuando el de al lado con más texto obliga a la fila del grid a estirarse) no quedaba centrado verticalmente, sólo horizontalmente. Se cambió a `display: flex; align-items: center; justify-content: center;` en los botones de acción del ticket (incluye `.ticket-print-actions a` en general, `.table-close-launcher`/`.confirm-close-button` y `.table-split-launcher` en Mesas), para que el texto quede centrado en ambos ejes sin importar la altura real de la celda.
- **Mesas: los tres botones de imprimir suben arriba del ticket**, justo como ya se hizo para Pedidos el mismo día — ahora van debajo del encabezado "Ticket" y arriba de la lista de artículos, en vez de hasta abajo después del total. Sólo se reordenó ese bloque dentro de `tables/templates/tables/table_detail.html` (sigue siendo hijo directo de `.table-ticket`, así que ningún selector CSS existente se rompió por el cambio de posición). "Cobrar y cerrar cuenta" se quedó donde estaba, hasta abajo — el desarrollador sólo pidió mover los botones de imprimir.
- Verificado visualmente con Playwright temporal (instalado, usado, desinstalado) a 1024px (tablet): la cuadrícula de Pedidos se ve compacta y pareja, y el ticket de Mesas muestra los tres botones de imprimir arriba, con "Bolillo" (el artículo) debajo.
- 113/113 pruebas siguen pasando (ningún cambio de este bloque tocó lógica de Python, sólo plantillas/CSS).

## Mismos botones: quitar el peso de letra "negritas" para que todos se vean como "Agregar nota general" (2026-09-24)

- El desarrollador reportó que, aun con la tipografía ya unificada (Nunito Sans en todos), "Cerrar captura y enviar al listado" e "Imprimir cocina" se seguían viendo en negritas comparados con "Agregar nota general" — pedía que todos compartieran exactamente el mismo peso de letra que ese botón.
- Todos estos botones ya tenían el mismo `font-weight` numérico (850 los de Pedidos y los de imprimir, 800 "Cobrar y cerrar cuenta"/"Dividir cuenta" en Mesas) — confirmado con Playwright antes de tocar nada. Se bajó ese valor de forma pareja a `600` en los seis lugares: `.internal-ticket-quick-actions` (la cuadrícula de Pedidos), `.ticket-print-actions a` (imprimir, compartido con Mesas), `.ticket-general-note-button`, `.table-close-launcher`/`.confirm-close-button` y `.table-split-launcher` en Mesas.
- Verificado con Playwright temporal (instalado, usado, desinstalado): los seis botones de la cuadrícula de Pedidos miden `font-weight: 600` de forma consistente y ya no se ven en negritas.

## Corrección: "Cobrar y cerrar cuenta" en Mesas también debía subir arriba, no sólo los botones de imprimir (2026-09-24)

- El desarrollador aclaró que al mover los botones de imprimir arriba del ticket en Mesas, "Cobrar y cerrar cuenta" (y el resumen de pago cuando la cuenta ya está cerrada) también debía subir — igual que en Pedidos subieron tanto "Guardar sin cerrar"/"Cerrar captura..." como los de imprimir juntos.
- Se movió ese bloque (`{% if account.status == 'open' %}` botón / `{% else %}` resumen de pago cerrado) de hasta abajo del ticket a justo debajo del encabezado "Ticket", ahora ANTES de los tres botones de imprimir (orden final: encabezado → Cobrar y cerrar cuenta (o resumen de pago) → imprimir → lista de artículos → total).
- Verificado con Playwright temporal: el botón queda arriba, seguido de los tres de imprimir, con el detalle de productos debajo. 113/113 pruebas siguen pasando.

## Repartos: fichas con frontera clara en tablet/celular, y control de estado completo (Admin + Telefonista) (2026-09-24)

- **Rediseño visual de las fichas**: en tablet y celular no quedaba claro dónde empezaba y terminaba cada pedido — el borde del `.card` base (1px, muy pálido) se perdía contra las líneas internas entre secciones (Cobro/Entrega/etc.), que usaban un color casi igual de sutil. Verificado con Playwright temporal en 900px (tablet) y 480px (celular) antes de tocar CSS, para confirmar el problema real en vez de adivinar. **Arreglo**: `.delivery-card` ahora lleva un acento grueso de 8px a la izquierda (mismo verde de marca, `#285943`) más un borde de 2px y una sombra sutil — mismo recurso visual que ya usan las fichas de Caja (`.cashier-order`) para separarse claramente unas de otras — y se aumentó el espacio entre fichas (`.delivery-grid` de `.65rem` a `1.1rem`). Las divisiones internas de cada ficha no se tocaron; sólo se reforzó la frontera exterior, que es la que se perdía.
- **Nuevo control de estado, para Admin y Telefonista (no Repartidor)**: petición original era "sólo Administrador", pero el desarrollador corrigió en la misma conversación a "Administrador y Telefonista, no tiene caso que lo vean Repartidores" — Meseros no aplica porque ese rol ni siquiera tiene acceso a `/app/repartos/` (`SECTION_ROLE_MATRIX`). Repartos ya tenía un botón acotado de "Marcar como en reparto/entregado" (visible para Admin y Telefonista, sólo cubre despachar/completar); se agregó un control APARTE con el mismo mecanismo que ya usa Caja (`_order_actions_for_user`/`orders:order_resolve`), que ofrece cualquier transición válida según el estado (confirmar, empezar a preparar, listo, etc.), gated con `{% if not is_delivery_profile %}` en vez de admin-only.
  - `orders/views.py: delivery_board` calcula `order.quick_action`/`order.quick_action_label` igual que Caja, sólo para quien no sea perfil Repartidor.
  - `orders/templates/orders/delivery_board.html`: nuevo bloque `.delivery-quick-status` con su propio formulario (`data-delivery-quick-status-form`) hacia `orders:order_resolve`.
  - `static/js/delivery-board.js`: nuevo manejador AJAX dedicado — **no** se reutilizó el manejador existente de `[data-delivery-status-form]` porque ese endpoint (`delivery_complete`) devuelve `next_label`, mientras que `order_resolve` devuelve `next_action`/`next_action_label` (nombres de campo distintos); compartir el mismo manejador habría hecho que el botón desapareciera después de cada clic en vez de avanzar a la siguiente transición.
  - **Pruebas nuevas** (`orders/tests.py::DeliveryBoardQuickStatusControlTests`, 4 pruebas): Admin ve el control; Telefonista también lo ve; Repartidor (aunque el pedido esté asignado a él) no lo ve; Admin puede avanzar el estado end-to-end vía `order_resolve`.
- 117/117 pruebas pasando. Sin commitear.

## Corrección: "Capturando" (DRAFT) no tenía ninguna acción disponible en el control rápido de estado (2026-09-24)

- El desarrollador reportó, probando el nuevo control de Repartos, que un pedido en "Capturando" no dejaba avanzar el estado manualmente. Causa: `available_order_actions()` nunca contempló el estado `DRAFT` — devolvía una lista vacía, así que tanto Caja como Repartos mostraban "Sin acción pendiente" para cualquier pedido varado ahí.
- Normalmente un pedido en Capturando avanza cuando el telefonista cierra la captura desde `/app/pedidos/<id>/editar/` (botón "Cerrar captura y enviar al listado"), que corre validaciones propias antes de dejarlo pasar: al menos un artículo, ningún paquete incompleto, nombre del cliente, fecha/hora de entrega, y para Entrega a domicilio además calle/número y forma de pago. Un pedido puede quedarse varado en Capturando si nadie llegó a cerrar esa captura.
- **Arreglo, sin saltarse esa validación**: se agregó una acción nueva `"close_draft"` (misma etiqueta: "Cerrar captura y enviar al listado") que `transition_order()` enruta directo a `close_internal_order_capture()` — el mismo código que ya corre ese botón — en vez de usar el mapa genérico de transiciones. Si al pedido le falta algo, la acción rápida falla con el mismo mensaje de validación que ya conocía el telefonista; sólo si todo está completo, avanza (a Programado o En preparación, según `scheduled_initial_status`).
- Como esta acción sale de `available_order_actions()`, queda disponible automáticamente en TODOS los controles que ya reutilizan ese mecanismo — Caja y el nuevo control de Repartos — sin tocar esas vistas.
- **Pruebas nuevas** (`orders/tests.py::DraftOrderQuickCloseTests`, 2 pruebas): un pedido Capturando completo (con artículo) sí avanza de estado vía la acción rápida; uno vacío se sigue bloqueando con el mismo mensaje de "agrega al menos un producto", confirmando que la validación no se saltó.
- 119/119 pruebas pasando. Sin commitear.

## "Marcar como no pagado": ya no exige que el pedido esté Entregado/Recogido, y se agregó a los "•••" de Pedidos y Caja (2026-09-25)

- El desarrollador reportó un error al intentar dejar un pedido a cuenta: "Vincula un cliente de la agenda antes de dejar este pedido a cuenta" — el cliente SÍ existía y aparecía en el buscador de la agenda, sólo que nunca quedó vinculado a ESE pedido en particular (typeó el nombre libremente sin seleccionar la sugerencia). Esa parte de la validación se dejó igual — sigue haciendo falta un cliente de agenda vinculado, el desarrollador no pidió quitarla — pero de ahí surgió pedir relajar la otra condición que sí bloqueaba: el estado del pedido.
- **Antes**: `create_customer_debt()` exigía que el pedido estuviera Entregado o Recogido (completando en automático un "En reparto" antes de dejarlo a cuenta). **Ahora**: cualquier estado es elegible, salvo Cancelado — sin transicionar el pedido a otro estado por sí sola.
- **Nuevo lugar para usarlo**: antes sólo existía el buscador por folio en Caja (`cashier-unpaid-quick`) y un botón suelto en Cambios pendientes. Se agregó "No pagó" como opción extra dentro del menú "•••" que ya existía en cada ficha de Pedidos (`orders/templates/orders/includes/order_board_item.html`, junto a "Cancelar pedido") — y ese mismo menú "•••" se agregó también a Caja, dentro de `cashier_status_action.html` (el include compartido que ya usan las tarjetas de entrega y de recoger), para no duplicar la marcación en dos plantillas distintas.
- **Bug encontrado al agregar el menú a Caja**: el listener de clic que abre el pedido al tocar la tarjeta (`static/js/cashier-board.js`) no excluía `details`/`summary` de sus elementos ignorados — tocar "•••" en Caja habría abierto el menú Y navegado al pedido al mismo tiempo. Pedidos ya tenía esa exclusión bien puesta (`static/js/order-list.js`); se igualó la lista en Caja.
- Visible sólo para Administrador (mismo rol que ya exigía la vista `cashier:order_debt_create`); en Cambios pendientes (`cashier_change_board.html`) se actualizó la misma condición de estado del botón "No pagó" que ya existía ahí.
- **Pruebas nuevas** (`orders/tests.py::MarkOrderAsUnpaidTests`, 5 pruebas): un pedido "En preparación" ya se puede dejar a cuenta; uno Cancelado se sigue rechazando con el mensaje nuevo; una entrega "En reparto" ya no se transiciona sola a Entregado al dejarla a cuenta; el botón aparece en la ficha de Pedidos y en la tarjeta de Caja.
- 124/124 pruebas pasando. Sin commitear.

## "Cancelar pedido" y "No pagó" se veían idénticos en el menú "•••" (2026-09-26)

- El desarrollador mandó una captura mostrando ambos botones en rojo dentro del mismo menú desplegable, sin forma de distinguirlos de un vistazo. "Cancelar pedido" se dejó como estaba (rojo, `.danger`, es la acción más destructiva); se agregó una clase nueva `.warning` (mismo tono ámbar/marrón que ya usa la app para "nota"/"admin") para "No pagó", que es una acción distinta — registra un adeudo, no cancela nada.
- Se aplicó en los tres lugares donde vive ese botón: el menú "•••" de Pedidos, el mismo menú agregado a Caja, y el botón suelto ya existente en Cambios pendientes.
- Verificado visualmente con Playwright temporal: ahora "Cancelar pedido" es rojo y "No pagó" es ámbar, claramente distinguibles. 124/124 pruebas siguen pasando (cambio puramente visual). Sin commitear.

## Pedidos: los botones −/+/×/Nota del ticket quedaban muy separados del producto en escritorio/tablet horizontal (2026-09-26)

- El desarrollador reportó que, al agregar el primer producto a un pedido de entrega/recoger, los botones de cantidad y nota quedaban flotando muy por debajo del artículo — sólo pasaba en escritorio y tablets en horizontal, en celular se veía bien.
- Diagnosticado con Playwright temporal midiendo estilos computados en vez de adivinar: el panel de ticket (`.table-ticket-items`) tiene una altura mínima para no verse vacío con pocos artículos — pero como sus filas de cuadrícula usaban el `align-content` por defecto (equivalente a "stretch"), con un solo producto esa única fila se estiraba para llenar toda esa altura mínima. Ese estiramiento se propagaba hacia adentro: la fila interna del artículo que contiene los botones también se estiraba, dejando los botones centrados verticalmente muy abajo, lejos del nombre del producto.
- Mesas ya tenía exactamente este mismo arreglo (`.table-pos-layout .table-ticket .table-ticket-item { align-items: start !important; ... }`) desde antes, pero nunca se replicó para Pedidos — en celular no se notaba porque los `@media` angostos ya fijaban estas mismas propiedades por separado.
- **Arreglo**: se agregaron dos reglas nuevas, sólo para Pedidos (`.internal-live-ticket > .table-ticket-items { align-content: start; }` y `.internal-live-ticket .internal-ticket-item { align-items: start; grid-auto-rows: max-content; }`), replicando el mismo criterio que ya usa Mesas.
- Verificado con Playwright: antes la fila de botones media 172px de alto (centrados ahí dentro, muy abajo); después mide 36px y queda pegada justo debajo del producto. 124/124 pruebas pasando. Sin commitear.

## Ticket de Mesas y Pedidos: el panel se estiraba a casi toda la pantalla aunque tuviera pocos artículos, tapando el "Total" (2026-09-26)

- El desarrollador reportó que el ticket, al abrirlo con pocos productos, "nace" ocupando casi toda la altura de la pantalla, empujando el "Total" fuera de la vista sin deslizar — y aclaró que pasa en celular, tablet y escritorio por igual, tanto en Pedidos como en Mesas.
- Diagnosticado con Playwright temporal midiendo estilos computados en varios anchos de pantalla (390, 900 y 1280px): el panel del ticket (`.internal-live-ticket` en Pedidos, `.table-pos-layout .table-ticket` en Mesas) tenía, repetido en al menos 9 bloques `@media` distintos acumulados a lo largo del archivo (celular, tablet vertical, tablet horizontal táctil, iPad Air específico, escritorio con mouse, etc.), una propiedad `height: calc(100dvh - Nrem) !important` fija — no sólo un tope (`max-height`), sino una altura forzada — además de que su lista de artículos (`.table-ticket-items`) usaba `flex: 1 1 ...` (flex-grow) o `flex: 1 1 auto` en varios de esos mismos bloques, lo que la hacía estirarse para rellenar todo ese espacio fijo aunque sólo hubiera un producto.
- **Arreglo**: en los ~9 bloques encontrados (algunos eran duplicados exactos, reflejo de la acumulación histórica ya conocida de este archivo) se quitó la línea `height: ...` dejando sólo `max-height` como tope real, y se cambió `flex: 1 1 ...` a `flex: 0 1 auto` (o `0 0 auto`) en la lista de artículos para que crezca según su contenido en vez de rellenar el espacio disponible. También se redujo de `16.5rem` a `4.5rem` el alto mínimo forzado que tenía sólo Pedidos-escritorio para la lista de artículos (agregado antes para que el panel de Efectivo no la aplastara a 0px) — un mínimo pequeño sigue evitando ese aplastamiento sin reservar espacio de más con pocos productos.
- Verificado con Playwright en 390×844 (celular), 900×1200 (tablet) y 1280×800 (escritorio), en ambas apps: el panel ahora mide su contenido real (por ejemplo, Pedidos-escritorio con 1 artículo pasó de 768px de alto fijo a ~496px) y el "Total" queda visible sin deslizar en los seis casos. 124/124 pruebas pasando (cambio puramente visual). Sin commitear.

## Mapa de mesas: ahora se ve el total consumido de cada mesa ocupada sin entrar al ticket (2026-09-26)

- El desarrollador pidió ver el total de la mesa directamente en `/app/mesas/`, sin tener que tocar "Continuar ticket" para averiguarlo.
- Se anotó `current_total=Sum("items__subtotal")` en el queryset de cuentas abiertas de `table_map` (`tables/views.py`), reutilizando el mismo patrón que ya usaba el historial de mesas (`table_account_history`) para sumar el total desde los artículos de la cuenta.
- En la tarjeta de cada mesa ocupada (`table_map.html`) se agregó un nuevo elemento `<span class="table-tile-total">` entre el mesero/hora y el botón "Continuar ticket", con una clase CSS nueva (`.table-tile-total`, en `app.css`) que lo resalta en el mismo tono que usa la tarjeta para "Ocupada". Se agregó también su tamaño reducido en el breakpoint móvil existente (≤620px) donde las tarjetas ya son compactas.
- 15/15 pruebas de `tables` pasando. Sin commitear.

## Mesas: encabezado del ticket ("Ticket" + "N artículos") con demasiado espacio arriba y entre ambos textos, sólo en escritorio (2026-09-26)

- El desarrollador reportó, con captura, que en `/app/mesas/cuentas/<id>/` el título "Ticket" queda muy separado del borde superior de la tarjeta, y "N artículos" queda muy separado de "Ticket" — sólo en escritorio/tablet, no mencionó problema en celular.
- Encontrado por lectura estática del CSS (sin Playwright, ya que el patrón era claro): el encabezado compacto en una sola línea (`.current-ticket-heading { display:flex; align-items:baseline; gap:.5rem; margin:0 0 .45rem; }` + `h2 { margin:0; font-size:1.15rem }` + `span { margin:0; font-size:.72rem }`) ya existía, pero **sólo estaba escrito para Pedidos** (`.internal-live-ticket > .current-ticket-heading`), nunca se replicó para Mesas. Sin esa regla, el `<h2>"Ticket"` en Mesas usaba el margen por defecto del navegador (~0.83em, más grande en escritorio por el tamaño de fuente heredado) y el `<span>"N artículos"` quedaba forzado a su propia línea debajo por ser bloque+inline sin flex.
- **Arreglo**: se replicó el mismo bloque de reglas, ahora también para `.table-pos-layout .table-ticket > .current-ticket-heading`, dejando "Ticket" y "N artículos" en una sola línea, pegados, igual que en Pedidos. Sin verificación visual con Playwright esta vez (patrón conocido y de bajo riesgo); pendiente de que el desarrollador lo confirme manualmente.

## Mapa de mesas: el botón "Continuar ticket" y el título "Mesa #" quedaban al ras del borde de la tarjeta, sólo en escritorio (2026-09-26)

- El desarrollador pidió, con captura, más espacio: el botón "Continuar ticket" se veía pegado al borde inferior de la tarjeta, y el título "Mesa #" pegado al borde superior.
- El padding de `.table-main-action` (el enlace que envuelve todo el contenido de una mesa ocupada) era de `1rem`; se aumentó a `1.4rem 1.1rem` (más aire arriba/abajo). Como el contenido interno está centrado verticalmente (`justify-content: center`), este padding extra separa por igual el título del borde superior y el botón del borde inferior.
- Cambio puramente visual, de bajo riesgo; pendiente de confirmación manual del desarrollador.

### Corrección: el padding de arriba no tuvo ningún efecto visible (2026-09-26)

- El desarrollador confirmó con otra captura que, en escritorio, "Mesa #" y "Continuar ticket" se seguían viendo exactamente igual de pegados a los bordes — el aumento de padding no cambió nada.
- Causa real: `.table-main-action` usa `justify-content: center` dentro de una altura fija (`height: 100%`, heredada del tamaño de la mesa en el mapa). Cuando el contenido se centra dentro de una altura fija, subir el padding arriba/abajo no mueve el hueco visible — el espacio extra que reparte el centrado ya compensa exactamente lo que el padding "quita", y el resultado matemático es el mismo hueco que sin el cambio. Por eso el ajuste anterior no se notó.
- **Arreglo correcto**: para la ficha ocupada (`.table-tile.occupied .table-main-action`) se cambió el acomodo de `center` a `flex-start`, y se le puso `margin-top: auto` al botón "Continuar ticket" (el truco estándar de flexbox para fijar el último elemento hasta el fondo sin centrar todo el contenido). Así el padding-top sí controla el hueco de "Mesa #" con el borde superior, y el padding-bottom sí controla el hueco del botón con el borde inferior.
- Sin verificación visual con Playwright (cambio de bajo riesgo, patrón de flexbox conocido); pendiente de confirmación manual del desarrollador.

### Segunda corrección: el botón quedó recortado, peor que antes (2026-09-26)

- El desarrollador mandó otra captura: ahora "Continuar ticket" apenas se asoma como una rebanada roja en el borde inferior de la tarjeta, prácticamente invisible — peor que el problema original.
- Esta vez sí se usó Playwright temporal para medir en vez de seguir adivinando (dos intentos fallidos seguidos). Medición real: la tarjeta (`.table-tile`, con `overflow: hidden`) mide 255px de alto en escritorio a 800px de viewport; el contenido interno (título, "Ocupada", mesero/hora, total, botón, más 4 espacios `gap` entre ellos) ya ocupaba ~213px; sumando el padding aumentado en el arreglo anterior (1.4rem × 2 = 44.8px) el total requerido (~258px) **superaba** los 255px disponibles — el botón se recortaba por el `overflow: hidden` de la tarjeta. El primer intento de padding (1rem) no se notó porque quedaba centrado (ver nota anterior); este, al pasar a `flex-start`, sí se notó — pero como diferencia negativa (recorte), no positiva.
- **Arreglo real**: la altura de las tarjetas del mapa de mesas depende de `.table-map`'s `height: calc(100dvh - Npx)` dividida entre 2 filas — no había margen real para más padding. Se le dio más presupuesto de altura al mapa completo (`height: calc(100dvh - 235px)`, antes 270px) y se subió su `min-height` de 330px (insuficiente para 2 filas de contenido) a `545px` (el mínimo real que necesitan 2 filas con el contenido + padding actuales, con margen de seguridad) para que en pantallas bajas el mapa haga scroll en vez de recortar el contenido de cada tarjeta.
- Verificado con Playwright en 700, 800, 900 y 1080px de alto de viewport: el hueco arriba de "Mesa #" y abajo de "Continuar ticket" es de 23.4px, igual en los cuatro casos, sin recortes. 15/15 pruebas de `tables` pasando.

## Mapa de mesas: el botón "Continuar ticket" se veía grande y con el texto partido en dos líneas en iPhone SE (2026-09-26)

- El desarrollador mandó una captura a 375×667 (iPhone SE): con 3 fichas por fila (regla ya existente para teléfonos, en `@media (max-width: 620px)`), el botón "Continuar ticket" seguía usando el tamaño de fuente y padding pensados para escritorio, así que el texto se partía en dos líneas y se veía desproporcionado frente al resto de la ficha, que sí tiene sus propios ajustes de tamaño para ese ancho.
- Se notó que esa misma media query de 620px ya reduce la fuente de "Mesa #", "Ocupada", mesero/hora y el total — pero nunca se agregó un ajuste equivalente para el botón "Continuar ticket".
- **Arreglo**: se agregó, en ese mismo bloque `@media (max-width: 620px)`, un padding y gap más compactos para `.table-main-action` y, específicamente para `.table-tile.occupied .table-main-action strong` (el botón), una fuente más chica (`.7rem`) y padding más ajustado, siguiendo el mismo criterio que ya usan sus elementos vecinos en ese ancho.
- Cambio de bajo riesgo y puramente reductivo (sólo achica, no agranda nada), aplicado en un contenedor cuya altura en móvil ya es automática (no fija, a diferencia de escritorio) — no hay riesgo de recorte como en el caso anterior. Sin verificación con Playwright esta vez; pendiente de confirmación manual del desarrollador. 15/15 pruebas de `tables` pasando.

## Repartos: "Desde" y "Hasta" quedan uno abajo del otro; el desarrollador pidió ponerlos en la misma fila, mitad y mitad (2026-09-26)

- El desarrollador pidió, con capturas de celular (iPhone SE) y escritorio, que los filtros de fecha "Desde" y "Hasta" en `/app/repartos/` queden en la misma fila (50/50) en vez de uno debajo del otro, y que el cambio aplique en celular, tablet y escritorio por igual.
- Causa: `.delivery-date-row` (el `<div>` que envuelve ambos `<label>`) nunca tuvo ninguna regla CSS propia — no existía en absoluto en `app.css`. Sin `display:flex/grid`, cada `<label>` se comportaba como bloque normal porque la regla global `input { width: 100%; }` hacía que el input de cada uno ocupara todo el ancho disponible, empujando el segundo `<label>` a su propia línea.
- **Arreglo**: se agregó `.delivery-date-row { display: flex; gap: .6rem; }` con `.delivery-date-row label { flex: 1 1 0; min-width: 0; flex-direction: column; ...}`, sin ninguna media query — así ambos campos quedan mitad y mitad en una sola fila en cualquier ancho de pantalla, tal como se pidió. Se revisó que ninguna media query existente de Repartos (820px, 620px) toque `.delivery-date-filter`/`.delivery-date-row`, así que no hay conflicto en ningún breakpoint.
- Sin verificación con Playwright (cambio sencillo y de bajo riesgo: flexbox con `flex:1 1 0` en ambos lados, sin contenedor de altura fija). 68/68 pruebas de `orders` pasando. Pendiente de confirmación manual del desarrollador.

## Caja: "Registrar un pedido que no pagó" no hacía nada al hacer clic, y los 6 botones de la fila se veían muy en negritas (2026-09-26)

- El desarrollador reportó, con captura de escritorio, que el botón "Registrar un pedido que no pagó" (en la fila junto a "Cuentas por cobrar", "Corte de terminales", etc.) no respondía al clic, y que los 6 botones de esa fila se veían con una tipografía muy en negritas, pidió una más fácil de leer.
- **Bug funcional (causa real)**: ese botón en realidad es un `<details><summary>` nativo (no un enlace) — JS (`cashier-board.js`) lo mueve dentro de la fila de herramientas en tiempo de ejecución para que se vea como los demás. En escritorio (`min-width: 901px`), su formulario emergente (Folio/Nota/Dejar a cuenta) se posiciona con `position: absolute`, pero el propio `<details>` conservaba `overflow: hidden` (heredado de su estilo base para otros contextos) — como `<details>` es también el contenedor posicionado (`position: relative`) del formulario absoluto, ese `overflow: hidden` recortaba el formulario a la altura colapsada del botón, dejándolo invisible aunque técnicamente sí se abría. Por eso "no hacía nada" a simple vista.
- **Arreglo del bug**: se agregó `overflow: visible` a `.cashier-page-header .cashier-unpaid-quick` en el bloque `@media (min-width: 901px)`. Verificado con Playwright: antes del cambio el formulario no era localizable/visible; después, `details.open === true` y el formulario aparece con sus campos "Folio"/"Nota" y el botón "Dejar a cuenta".
- **Bug secundario encontrado al verificar**: una vez visible, el formulario (pensado originalmente para una fila ancha) usaba 3 columnas fijas (190px + 300px + 74px ≈ 564px) dentro de un popover de sólo 420px de ancho, así que el botón "Dejar a cuenta" quedaba fuera de la pantalla. Se agregó `grid-template-columns: 1fr 1fr` (Folio y Nota lado a lado) con el botón ocupando toda la fila de abajo (`grid-column: 1 / -1`), reutilizando el mismo patrón que ya existía para pantallas angostas. También se ajustó la posición del texto de ayuda de abajo (`top: calc(100% + 9.6rem)` → `11.2rem`) para que no quedara encimado con el formulario, ahora más alto por el cambio a dos filas.
- **Tipografía**: se bajó el peso de fuente de `.cashier-tools a` (los 5 enlaces) de `750` a `600`, y el de `.cashier-unpaid-quick summary` de `900` a `600` — ambos ya usaban la fuente "Nunito Sans" (la misma que se usó para el resto de botones operativos esta sesión), así que no hacía falta cambiar la familia tipográfica, sólo aligerar el grosor.
- 68/68 pruebas de `orders` pasando.

## Pedidos: "Limpiar formulario" en Datos del cliente, Enter cerraba el pedido por accidente, y el pago no se apagaba en Entrega (2026-09-26)

- El desarrollador pidió tres cosas sobre el panel "Datos del cliente" en `/app/pedidos/#/editar/` (funciona igual en Recoger y en Entrega a domicilio):
  1. Un botón nuevo "Limpiar formulario" que borre todo lo capturado (nombre, teléfono, fecha/hora de entrega, domicilio, notas).
  2. Reportó que al escribir en cualquier campo del panel (nombre, teléfono, fecha, hora, etc.) y presionar Enter, el pedido se cerraba en vez de sólo llenar el campo.
  3. Preguntó si el formulario realmente autoguarda cada campo — confirmado que sí: cada input/select/textarea del panel dispara un guardado automático (con 650ms de espera) hacia el mismo endpoint de autoguardado, tal como indica el texto "Datos guardados automáticamente" que ya existía.
- **Causa del Enter**: el `<form>` de captura (`#internal-order-data-form`) no tiene ningún `<button type="submit">` como descendiente propio — los dos botones reales de envío ("Guardar sin cerrar" y "Cerrar captura y enviar al listado") viven físicamente en el ticket, asociados sólo por el atributo `form="internal-order-data-form"`. Sin un botón de envío dentro del formulario, el navegador resolvía el envío implícito por Enter de forma poco predecible, terminando en "Cerrar captura" en vez de simplemente no enviar nada.
- **Arreglo del Enter**: se agregó un listener de `keydown` sobre el panel que bloquea el Enter en cualquier `<input>` de una sola línea (el `<textarea>` de notas sigue permitiendo Enter para salto de línea, ya que nunca dispara un envío por sí solo).
- **Botón "Limpiar formulario"**: se agregó en `orders/templates/orders/internal_order_form.html`, junto a "Guardar datos"/"Seguir agregando productos", con la clase `.danger` para distinguirlo como acción destructiva. Pide confirmación (`window.confirm`, mismo patrón ya usado en `customer-list.js`), borra todos los campos del panel MÁS los campos ocultos `agenda_customer_id`/`agenda_address_id` (que viven fuera del panel pero deben limpiarse junto con el resto, ya que están ligados al cliente seleccionado), y guarda ese estado vacío de inmediato con el mismo autoguardado existente.
- **Bug adicional encontrado y corregido, relacionado**: el desarrollador reportó después, en el mismo hilo, que el método de pago "Efectivo" se apagaba al tocarlo dos veces sólo en modalidad Recoger, pero no en Entrega a domicilio. Revisando el código se confirmó que esa restricción (`if (orderType === "pickup" && field?.checked)`) era intencional pero ya no se quería — se quitó la condición de modalidad para que apagar el método de pago tocándolo de nuevo funcione igual en ambas.
- Verificado con Playwright temporal (orden #191, Entrega a domicilio): 1) tocar "Efectivo" dos veces ahora lo apaga; 2) escribir en "Nombre del cliente" y presionar Enter ya no navega ni cierra el pedido, conserva el texto; 3) "Limpiar formulario" deja nombre/teléfono/domicilio vacíos. 68/68 pruebas de `orders` pasando.

## Pedidos (escritorio): el ticket con muchos artículos se veía apretado, "Efectivo"/propina tenían su propio scroll interno, y el scroll de dos dedos no funcionaba sobre los artículos (2026-09-26)

- El desarrollador reportó, con capturas, tres problemas relacionados en el ticket de Pedidos en escritorio cuando hay varios productos: 1) mucho margen desperdiciado antes de "Total"/"Pago"; 2) el panel parecía de altura fija, sin crecer para mostrar al menos 5 artículos antes de necesitar scroll; 3) al expandir "Efectivo" (billetes) o la propina de "Transferencia", aparecía un scroll interno propio de ese panel que no debería existir.
- Diagnosticado con Playwright temporal midiendo alturas reales: `.ticket-payment-details` (el contenedor de Efectivo/propina, sólo en escritorio con mouse) tenía `max-height: 12rem !important` con su propio `overflow-y: auto` — un arreglo de una sesión anterior para evitar que Efectivo aplastara la lista de artículos a 0px, pero que creaba un scroll anidado en vez de dejar que el panel completo del ticket (que ya tiene su propio `overflow-y: auto` para estos casos) se encargara con un solo scroll. Además, la lista de artículos sólo tenía `min-height: 4.5rem` (~1 renglón), muy por debajo de los 5 que se pidieron.
- **Arreglo**: se quitó el `max-height`/`overflow-y` propio de `.ticket-payment-details` (ahora se muestra completo, sin scroll interno) y se subió el `min-height` de la lista de artículos de `4.5rem` a `28rem` (~5 renglones reales, medido con datos de prueba). Con esto, "Total"/"Pago" quedaron pegados justo debajo de los artículos (antes el hueco variaba de forma inconsistente por cómo se repartía el espacio en el layout flexible), y cuando el contenido total no cabe, es ahora el panel del ticket completo el que se desplaza como una sola barra lateral, en vez de tener scrolls anidados.
- **Bug de scroll de dos dedos (reportado aparte, mismo hilo)**: el desarrollador señaló que el scroll de dos dedos no funcionaba al posicionar el mouse sobre los artículos del ticket, en Mesas y en Pedidos por igual. Causa: la lista de artículos (`.table-ticket-items`) tenía `overscroll-behavior: contain` en 7 lugares distintos del archivo (repetido en varios `@media` de escritorio/tablet, el patrón de acumulación ya conocido este archivo) — esa propiedad, aunque el elemento no tenga nada que desplazar internamente, IMPIDE que el gesto de scroll se propague ("encadene") hacia el contenedor padre (el panel completo del ticket), literalmente "atrapando" el scroll sin hacer nada. Se cambiaron las 7 ocurrencias de `contain` a `auto` específicamente en la lista de artículos (dejando `contain` sin tocar en el panel del ticket completo, que sí debe evitar que el scroll se filtre hacia toda la página). Se revisaron aparte las demás ocurrencias de `contain` en el archivo (diálogos modales y menús desplegables) y se dejaron intactas porque ahí sí es el comportamiento correcto.
- Verificado con Playwright: `.ticket-payment-details` ya no tiene su propio límite de altura; la lista de artículos mide 448px (min-height nuevo) sin desbordarse con 5 artículos; el panel completo del ticket sí mide más contenido del que cabe y usa su propio scroll unificado; los estilos computados confirman `overscroll-behavior: auto` en la lista de artículos y `contain` en el panel completo. El comportamiento real del scroll de dos dedos con trackpad no se pudo verificar de forma concluyente con Playwright (las herramientas de automatización no reproducen fielmente los gestos nativos de trackpad), pero la causa raíz en CSS es inequívoca y ya está corregida. 83/83 pruebas (`orders`+`tables`) pasando.

## Pedidos: rediseño del encabezado de "Datos del cliente" — se quitó "Seguir agregando productos" y "Limpiar formulario"/"Guardar datos" se movieron arriba (2026-09-26)

- El desarrollador pidió, para Recoger y Entrega a domicilio por igual: 1) eliminar el botón "Seguir agregando productos"; 2) mover "Limpiar formulario" y "Guardar datos" al encabezado del panel, a la izquierda del botón "×", achicándolos si hace falta; 3) que ambos usen la misma tipografía que tenía "Seguir agregando productos" pero sin negritas; 4) "Guardar datos" en verde-contorno (fondo blanco, texto y borde verde) y "Limpiar formulario" en rojo-contorno (fondo blanco, texto y borde rojo).
- Se movió el marcado de ambos botones al `<header>` del panel (`orders/templates/orders/internal_order_form.html`), envueltos en un nuevo `.customer-panel-header-actions` para que el encabezado siga funcionando con `justify-content: space-between` (título a la izquierda, botones+× agrupados a la derecha). Se quitó por completo el botón "Seguir agregando productos" y, de la fila inferior, sólo queda el texto "Datos guardados automáticamente".
- Nuevas clases `.customer-panel-header-clear`/`.customer-panel-header-save` en `app.css`: tamaño compacto (`min-height: 2.35rem`, `font-size: .7rem`), `font-weight: 600` (sin negritas) y los colores pedidos (contorno rojo/verde sobre fondo blanco). Se limpiaron las reglas que quedaron muertas en la fila inferior (`.customer-panel-actions button`/`.customer-panel-actions .secondary-action`), ya que ese contenedor ya no tiene botones.
- Verificado con Playwright y capturas en escritorio y celular: en escritorio ambos botones quedan en una sola fila junto al "×"; en celular, al no caber los tres, se acomodan en dos líneas de forma legible (sin superponerse). 83/83 pruebas (`orders`+`tables`) pasando.

### Corrección: el desarrollador pidió mantener también los botones abajo, más grandes, y el "×" en rojo (2026-09-26)

- El desarrollador aclaró que el cambio anterior no era lo que pedía: "Limpiar formulario" y "Guardar datos" debían **quedarse también abajo** (no sólo moverse arriba), el botón "×" debía ponerse en rojo, y ambos bloques de botones (arriba y abajo) debían verse un poco más grandes — del tamaño del botón "×".
- Se restauraron "Limpiar formulario" y "Guardar datos" en la fila inferior (`.customer-panel-actions`), reutilizando las mismas clases `.customer-panel-header-clear`/`.customer-panel-header-save` para que arriba y abajo se vean idénticos. Como ahora hay DOS botones "Limpiar formulario" en la página (arriba y abajo), se corrigió `internal-order-form.js`, que antes usaba `querySelector` (sólo engancha el primero que encuentra) — ahora usa `querySelectorAll(...).forEach(...)`, igual que ya hacía el botón "Guardar datos"/cerrar panel, para que el de abajo también funcione.
- Se subió el tamaño de ambos botones (`min-height` de `2.35rem` a `2.8rem`, mismo alto que el "×"; fuente de `.7rem` a `.8rem`; más padding) y se agregó `color: #a12622; border-color: #a12622;` al botón "×" para ponerlo en rojo, sobrescribiendo sólo ese botón específico (por `aria-label="Cerrar panel"`) sin afectar el resto de los estilos genéricos de botones del encabezado.
- Verificado con Playwright: el botón "Limpiar formulario" de abajo ahora sí dispara el borrado (antes no tenía ningún listener). 83/83 pruebas pasando.

## Ticket: el scroll de dos dedos no "encadenaba" hacia el scroll de la página cuando el ticket llegaba a su propio límite (2026-09-26)

- El desarrollador reportó, con captura, que al hacer scroll de dos dedos sobre el ticket, aunque ya había llegado al final del scroll interno del ticket, no podía ver "Total" porque el scroll de la página (el otro scroll, el de afuera) no se movía — preguntó si se podía detectar que se sigue intentando bajar y entonces mover ese scroll externo.
- Causa: el panel del ticket (`.internal-live-ticket`/`.table-pos-layout .table-ticket`) tenía `overscroll-behavior: contain` a nivel de todo el panel (en dos lugares del archivo). Igual que con la lista de artículos (arreglada antes en esta misma sesión), `contain` bloquea que el scroll se "encadene" hacia el siguiente contenedor desplazable una vez que el elemento actual llega a su propio límite — en este caso, ese "siguiente contenedor" es la página completa. Como el ticket usa `position: sticky`, una vez que queda fijo cerca del borde superior, su `max-height` garantiza que quepa completo en la pantalla — pero mientras la página todavía no ha hecho ese scroll inicial (el ticket aún no "se pega"), el usuario necesita que el scroll de la página avance primero, y `contain` se lo impedía.
- **Arreglo**: se cambió `overscroll-behavior: contain` a `overscroll-behavior: auto` en las dos reglas que gobiernan el panel completo del ticket (una para Pedidos+Mesas combinados, otra duplicada más antigua sólo para Pedidos). `auto` es el comportamiento estándar del navegador: al llegar al final del scroll interno, el gesto continúa moviendo el scroll del contenedor padre (la página) de forma natural, sin que el usuario tenga que reposicionar el mouse fuera del ticket. Se dejó `contain` sin tocar en los diálogos modales y menús desplegables del archivo, donde sí es el comportamiento correcto (evitar que interactuar con un modal mueva la página detrás).
- Cambio de bajo riesgo (usa el valor por defecto del navegador). 83/83 pruebas pasando.

## Corrección: "Limpiar formulario" de arriba seguía sin verse rojo, y ya no debe preguntar confirmación (2026-09-26)

- El desarrollador reportó que el botón "Limpiar formulario" de ARRIBA seguía sin verse rojo (el de abajo sí), y pidió además que al presionarlo ya no pregunte si se desea borrar — que borre directamente.
- Causa del color: `.customer-data-panel header button` (la regla genérica para cualquier botón dentro del `<header>`, pensada originalmente sólo para el "×") tiene más especificidad CSS que `.customer-panel-header-clear`/`.customer-panel-header-save` a solas, así que le ganaba el color oscuro genérico — pero sólo al botón de ARRIBA, porque el de abajo vive fuera de `<header>` y nunca compitió con esa regla. Se corrigió subiendo la especificidad de las reglas de color (`.customer-data-panel .customer-panel-header-clear`/`.customer-data-panel .customer-panel-header-save`), igual que ya se había hecho para el tamaño.
- Se quitó el `window.confirm(...)` que pedía confirmación antes de borrar — ahora "Limpiar formulario" borra de inmediato, en ambas ubicaciones (arriba y abajo), ya que comparten el mismo manejador de clic.
- Verificado con Playwright: color computado del botón de arriba ahora es `rgb(161, 38, 34)` (rojo, igual que el de abajo); 0 diálogos de confirmación al hacer clic; el campo se borra de inmediato. 83/83 pruebas pasando.

## Caja: el menú "•••" (Cancelar pedido / No pagó) se salía de la pantalla en iPhone SE y tablet horizontal (2026-09-26)

- El desarrollador reportó, con captura, que en `/app/caja/` el menú "•••" (agregado a Caja en una sesión anterior) se veía cortado/fuera de la tarjeta en iPhone SE y en tablet horizontal (TabA9+).
- Causa: `.order-more-actions-menu` se ancla con `right: 0` (se abre hacia la izquierda del botón "•••") — ese comportamiento está bien en Pedidos, donde el botón vive al final (derecha) de su fila. Pero en Caja, el botón vive al **inicio** (izquierda) de una columna angosta (`.cashier-status-action`, alineada con `flex-start`) — el menú, de 175px de ancho, se abría hacia la izquierda de un botón que ya estaba pegado al borde izquierdo, saliéndose de la pantalla.
- **Arreglo**: se agregó `.cashier-status-action .order-more-actions-menu { right: auto; left: 0; }`, sólo para Caja, para que ahí el menú se abra hacia la derecha (donde sí hay espacio dentro de la tarjeta), sin tocar el comportamiento existente en Pedidos.
- Verificado con Playwright en iPhone SE (375px, con la tarjeta expandida primero, ya que en compacto vive detrás de un acordeón) y tablet horizontal (1180px): el menú ya no se sale de la pantalla en ningún caso. 83/83 pruebas pasando.

## Corrección: el ticket con pocos artículos volvió a mostrar un hueco enorme antes de "Total" (2026-09-26)

- El desarrollador reportó, con capturas, que el arreglo anterior de "mínimo 5 filas visibles" tuvo un efecto secundario: con 1 solo artículo, quedaba un espacio en blanco enorme antes de "Total" — pidió que el ticket creciera con el contenido real (chico si hay pocos productos) y sólo topara en ~5 productos antes de generar scroll interno, "sin estar limitado desde el principio".
- Causa exacta del error propio: el arreglo anterior usó `min-height: 28rem` en la lista de artículos — un **piso** que siempre reserva ese espacio, sin importar cuántos productos haya. Lo que realmente se necesitaba era un **techo** (`max-height`), que sólo limita cuando el contenido de verdad excede esa altura, dejando que el elemento sea más chico con poco contenido.
- **Arreglo**: se cambió `min-height: 28rem` por `max-height: 28rem` en `.internal-live-ticket > .table-ticket-items` (sólo escritorio con mouse). Al hacer ese cambio se encontró un segundo efecto: con `max-height` nada más, el layout flexible seguía **encogiendo** la lista de artículos por debajo de esa altura cuando el resto del contenido del ticket (panel de pago, etc.) competía por espacio — así que se agregó `flex-shrink: 0` para que la lista de artículos mida exactamente su contenido (topado en 28rem), sin que otros elementos la aplasten; si aun así todo el contenido del ticket no cabe, es el panel completo el que se desplaza (con el arreglo de scroll unificado ya hecho antes en esta misma sesión).
- Verificado con Playwright: con 1 artículo, la lista mide 80px (su contenido real) y el hueco hasta "Total" es de 7px; con 5 artículos, la lista mide 443px (los 5 completos, sin recortar) con el mismo hueco de 7px hasta "Total", y el panel completo pasa a necesitar su propio scroll (antes esto llegaba a aplastar la lista a 43px). 127/127 pruebas (suite completa) pasando.

## Producción: error 500 al eliminar un producto usado en menús diarios o con existencias registradas (2026-09-26)

- El desarrollador reportó un error 500 real en producción (`https://supercocina.win/app/menu/productos/172/eliminar/`) al intentar eliminar un producto específico.
- **Causa raíz encontrada**: el modelo `Product` está protegido (`on_delete=models.PROTECT`, a propósito, para no perder histórico) desde 9 relaciones distintas: los 8 campos de componente de `DailyMenu` (agua, consomé, primer/segundo tiempo, guisados, complemento) y `DailyProductStock.product` (existencias de inventario diario). La vista `product_delete` llamaba `product.delete()` directo, sin capturar `ProtectedError` — cualquier producto que alguna vez se haya usado como componente de un menú diario, o que tenga un registro de existencias, tira esa excepción sin capturar, resultando en el 500 que vio el desarrollador (reproducido localmente de forma controlada: se confirmó que `DailyProductStock` sí dispara `ProtectedError` al intentar borrar su producto).
- **Arreglo**: se agregó `_product_delete_blocker(product)`, que revisa las 9 relaciones protegidas y devuelve una razón legible si el producto está en uso. La vista `product_delete` ahora usa esa función para: 1) mostrar la página de confirmación con el aviso de bloqueo ya desde el GET (mismo patrón que ya existía para categorías con productos), y 2) como red de seguridad, capturar `ProtectedError` alrededor de `product.delete()` en el POST, por si la condición cambia entre que se carga la página y se confirma. En ambos casos se muestra un mensaje de error normal en vez de una página de error 500.
- Se agregaron 3 pruebas nuevas (`menu/tests.py`, `ProductDeleteViewTests`) que reproducen exactamente el bug reportado (producto con `DailyProductStock`) y confirman que ahora la vista responde 200 con un mensaje de error, en vez de 500, además de un borrado normal para productos sin relaciones protegidas. 127/127 pruebas (suite completa) pasando.
- **Nota para el desarrollador**: este cambio ya está en el código local pero, como toda esta sesión, no se ha commiteado ni desplegado — para que deje de pasar en `supercocina.win` hace falta que subas este cambio a producción tú mismo, como de costumbre.

## Producción: error 500 al agregar el agua del día a un paquete desde "Bebidas frías" (2026-09-26)

- El desarrollador reportó otro 500 real en producción: en Pedidos (Recoger o Entrega a domicilio), agregar el agua del día desde la categoría "Bebidas frías" tiraba `django.db.utils.NotSupportedError: FOR UPDATE no puede ser aplicado al lado nulable de un outer join`.
- **Causa raíz**: en `add_water_to_internal_package` (`orders/services.py:870`), la consulta que busca la partida del paquete a la que agregar agua usaba `select_for_update().select_related("package")`. Como `OrderItem.package` es un FK nulable (`on_delete=SET_NULL`), `select_related` genera un LEFT OUTER JOIN, y PostgreSQL no permite bloquear (`FOR UPDATE`) el lado nulable de un outer join. Este es el mismo patrón que ya se había encontrado y corregido antes en `update_internal_package_extras` y en `create_customer_debt` (ambos con una nota en el código explicándolo) — sólo que esta función se quedó con el patrón viejo.
- **Arreglo**: se replicó exactamente la misma solución ya usada en esas otras dos funciones — bloquear (`select_for_update()`) sólo la partida (`OrderItem`), sin el `select_related`, y leer el paquete después en una consulta aparte sin bloqueo (`MealPackage.objects.get(pk=item.package_id)`).
- Se agregó una prueba de regresión (`orders/tests.py`, `AddWaterToPackageTests`) que reproduce el flujo real (POST a `internal_order_product_add`, la misma URL del error reportado) contra Postgres real (este proyecto usa Postgres tanto en desarrollo como en pruebas, así que la restricción de Postgres sí se ejerce). Se verificó manualmente que, revirtiendo el arreglo, la prueba falla con el mismo `NotSupportedError` exacto reportado — confirmando que la prueba sí habría atrapado este bug antes de llegar a producción. 128/128 pruebas (suite completa) pasando.
- **Nota para el desarrollador**: igual que el bug anterior, este cambio está listo en el código local pero no se ha commiteado ni desplegado — lo subes tú cuando quieras.
